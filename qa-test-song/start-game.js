(async () => {
  ...
  const setNative = (el, value, changeOnly) => {
  ...
  randomCell.click();
  await sleep(600);
  // 6. Interval 30s, final 60s
  const labels = Array.from(document.querySelectorAll('label'));
  const intLabel = labels.find(l => /Round Duration \(Elimination Interval\)/.test(l.textContent));
  if (!intLabel) return 'no-interval-label';
  setNative(intLabel.parentElement.querySelector('input[type=range]'), '30');
  await sleep(300);
  const finLabel = labels.find(l => /Final Round Duration/.test(l.textContent));
  const finSlider = finLabel?.parentElement.querySelector('input[type=range]');
  if (finSlider) { setNative(finSlider, '60'); await sleep(300); }
  // 7. Ready to Play
  ...
  await sleep(2000);
  // 8. Start Round 1
  const start = Array.from(document.querySelectorAll('button')).find(b => /Start Round 1/.test(b.textContent));
