...
export type GameMode = 'standard' | 'pass-the-mic' | 'companion-singalong' | 'medley' | 'missing-words' | 'duel' | 'blind' | 'tournament' | 'battle-royale' | 'duet';
...
export const DIFFICULTY_SETTINGS = {
  easy: {
    pitchTolerance: 2, // +/- 2 semitones within the octave (SingStar-style: C accepted for C, C#, D)
    timingTolerance: 300, // ms - more lenient timing
    noteScoreMultiplier: 0.8, // Lower points for easier difficulty
    comboMultiplier: 1.5,
    visualNoteWidth: 1.3,
    // Karaoke-optimized settings
    volumeThreshold: 0.05, // Lower volume threshold - easier to trigger
    pitchStabilityFrames: 3, // Require fewer stable frames for pitch detection
  },
  medium: {
    pitchTolerance: 1, // +/- 1 semitone within the octave (SingStar-style: C accepted for C, C#)
  ...
  hard: {
    pitchTolerance: 0, // EXACT pitch class required - must sing the exact semitone (but octave doesn't matter)
    timingTolerance: 100, // ms - stricter timing
    noteScoreMultiplier: 1.3, // Higher points for harder difficulty
    comboMultiplier: 2.5,
    visualNoteWidth: 0.8,
    // Strict settings
    volumeThreshold: 0.1, // Higher volume threshold - requires stronger singing
    pitchStabilityFrames: 7, // Require more stable frames
  },
} as const;
