...
const DEFAULT_CONFIG: MicrophoneConfig = {
  ...
  sampleRate: 44100,
  latency: 'interactive', // Lowest latency for karaoke
};
...
export class MultiMicrophoneManager {
  ...
  offAssignedMics(callback?: (mics: AssignedMicrophone[]) => void): void {
    // If specific callback provided, only remove if it matches
  ...
  private loadConfig(): void {
    try {
      ...
      if (saved) {
        ...
        // Migration: Convert old latency values in saved mic configs
        // Old: 'low', 'normal', 'high' -> New: 'interactive', 'balanced', 'playback'
        if (config.assignedMics && Array.isArray(config.assignedMics)) {
          config.assignedMics.forEach((mic: { config?: { latency?: string } }) => {
