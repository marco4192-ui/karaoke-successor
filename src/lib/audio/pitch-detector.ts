...
// Karaoke-optimized defaults
export const KARAOKE_DEFAULT_CONFIG: PitchDetectorConfig = {
  volumeThreshold: 0.05,          // Very sensitive - picks up quiet singing
  pitchStabilityFrames: 3,        // Require 3 stable frames (~50ms at 60fps)
  yinThreshold: 0.15,             // Standard YIN threshold
  noiseGateEnabled: true,
  noiseGateThreshold: -40,        // -40dB noise gate
  minFrequency: 65,               // C2
...
export const DIFFICULTY_PITCH_CONFIGS: Record<Difficulty, PitchDetectorConfig> = {
  easy: {
    volumeThreshold: 0.03,        // Very sensitive
    pitchStabilityFrames: 2,      // Quick response
    yinThreshold: 0.12,           // More lenient
    noiseGateEnabled: true,
    noiseGateThreshold: -45,      // Lenient noise gate
    minFrequency: 65,
    maxFrequency: 1047,
  },
  medium: {
  ...
  hard: {
    volumeThreshold: 0.08,        // Requires stronger singing
    pitchStabilityFrames: 5,      // More stable pitch required
    yinThreshold: 0.18,           // Stricter detection
    noiseGateEnabled: true,
...
export class PitchDetector {
  ...
  private yinPitchDetection(buffer: Float32Array, sampleRate: number, threshold: number = 0.15): number | null {
