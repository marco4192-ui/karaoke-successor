...
// IMPORTANT: Stores media files in IndexedDB for persistence in browser mode
export async function convertScannedSongToSong(scanned: ScannedSong): Promise<Song> {
  ...
  // Generate song ID BEFORE storing media (needed for IndexedDB key)
  const songId = `scanned-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  
  // Store media files in IndexedDB for persistence
  let storedMedia = false;
  
  try {
    if (scanned.audioFile) {
      await storeMedia(songId, 'audio', scanned.audioFile);
      console.log(`[FolderScanner] Stored audio for song ${songId} in IndexedDB`);
      storedMedia = true;
    }
    if (scanned.videoFile) {
      await storeMedia(songId, 'video', scanned.videoFile);
      console.log(`[FolderScanner] Stored video for song ${songId} in IndexedDB`);
      storedMedia = true;
    }
    if (scanned.coverFile) {
      await storeMedia(songId, 'cover', scanned.coverFile);
      console.log(`[FolderScanner] Stored cover for song ${songId} in IndexedDB`);
      storedMedia = true;
    }
  } catch (mediaErr) {
    console.warn(`[FolderScanner] Failed to store media for ${scanned.title}:`, mediaErr);
    // Continue without stored media - will use blob URLs for this session only
  }
  ...
  return {
    ...
    // IMPORTANT: Only set audioUrl if there's a SEPARATE audio file
    // If the video has embedded audio, leave audioUrl undefined
    ...
    storedMedia, // Flag indicating media is stored in IndexedDB
  };
}
