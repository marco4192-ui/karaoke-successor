    ...
    if (stored) {
      ...
      songs = songs.map((song: Song) => {
        if (song.hasEmbeddedAudio && song.audioUrl) {
          console.log(`[SongLibrary] Migrating song ${song.title}: clearing audioUrl for embedded audio video`);
          needsSave = true;
      ...
      if (needsSave) {
        try {
          ...
          console.log('[SongLibrary] Saved migrated songs to localStorage');
        } catch (e) {
...
function saveCustomSongs(songs: Song[]): void {
  try {
    ...
    // Try to save without blob URLs but keep metadata about stored media
    try {
      const minimalSongs = songs.map(s => {
        // Check if media is stored in IndexedDB
        const hasStoredMedia = s.storedMedia === true;
        
        return {
          ...s,
          // Keep the storedMedia flag so we know media exists in IndexedDB
          storedMedia: hasStoredMedia,
          // Remove blob URLs that don't persist, but keep track that they exist
          audioUrl: hasStoredMedia ? undefined : (s.audioUrl && !s.audioUrl.startsWith('blob:') ? s.audioUrl : undefined),
          videoBackground: hasStoredMedia ? undefined : (s.videoBackground && !s.videoBackground.startsWith('blob:') ? s.videoBackground : undefined),
      ...
      console.log('[SongLibrary] Saved minimal song data (media stored in IndexedDB)');
    } catch (e2) {
