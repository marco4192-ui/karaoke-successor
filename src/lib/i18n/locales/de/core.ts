// DE translations — core

export const coreTranslations = {
core: {
  branding: 'Karaoke ZERO',
},
nav: {
  library: 'Bibliothek',
  import: 'Import',
  party: 'Party',
  profiles: 'Profile',
  queue: 'Warteschlange',
  highscores: 'Highscores',
  jukebox: 'Jukebox',
  mobile: 'Mobil',
  achievements: 'Erfolge',
  daily: 'Täglich',
  settings: 'Einstellungen',
  editor: 'Editor',
},
home: {
  title: 'Karaoke ZERO',
  subtitle: 'Das ultimative Karaoke-Erlebnis. Singe dein Herz aus mit Echtzeit-Pitch-Erkennung, tritt gegen Freunde an und genieße Party-Spiele!',
  startSinging: 'Singen Starten',
  partyMode: 'Party-Modus',
  songsAvailable: 'Songs verfügbar',
  profilesCreated: 'Profile erstellt',
  partyGames: 'Party-Spiele',
  difficultyLevels: 'Schwierigkeitsstufen',
  selectProfile: 'Wähle dein Profil',
  createNew: 'Neu erstellen',
  qrCodeAlt: 'QR-Code',
},
homeScreen: {
  subtitle: 'Das ultimative Karaoke-Erlebnis. Singe dein Herz aus mit Echtzeit-Pitch-Erkennung, tritt gegen Freunde an und genieße Party-Spiele!',
  loadingStats: 'Lade Statistiken...',
  realTimePitch: 'Echtzeit-Pitch-Erkennung',
  realTimePitchDesc: 'Der fortschrittliche YIN-Algorithmus erkennt deine Sing-Pitch in Echtzeit mit hoher Genauigkeit. Sieh deine Stimme visualisiert!',
  partyGamesFeature: 'Party-Spiele',
  partyGamesFeatureDesc: 'Mikrofon weitergeben, Medley-Wettbewerb, Fehlende Wörter, Duell-Modus und Blind-Karaoke — endlose Unterhaltung für deine Partys!',
  mobileCompanion: 'Mobiler Companion',
  mobileCompanionDesc: 'Nutze dein Smartphone als Mikrofon oder Fernbedienung! Scanne den QR-Code zum Verbinden.',
  wifiStep1: '1. Gleiches WLAN-Netzwerk',
  wifiStep2: '2. QR-Code mit Kamera scannen',
  wifiStep3: '3. Link im Browser öffnen',
  detectingNetwork: 'Erkennen der Netzwerkadresse...',
  selectCharacter: 'Wähle deinen Charakter',
  inactiveProfiles: 'inaktive Profile ausgeblendet. Aktiviere sie in den Profileinstellungen.',
  // Daily/Weekly preview cards
  dailyTitle: 'Tages-Challenge',
  weeklyTitle: 'Wochen-Challenge',
  challengeLoading: 'Lade Challenges...',
  dailyProgress: '{done} von {total} Slots',
  slotLabel: 'Slot {n}',
  resetIn: 'Reset in {time}',
  dailyCta: 'Daily starten',
  weeklyCta: 'Weekly ansehen',
  allDailyDone: 'Alle Tages-Slots geschafft! 🎉',
  allWeeklyDone: 'Alle Wochen-Slots geschafft! 🎉',
  xpReward: '+{xp} XP',
  needProfileTitle: 'Profil erforderlich',
  needProfileDesc: 'Erstelle ein Spielerprofil, um Daily- und Weekly-Challenges zu verfolgen, XP zu sammeln und Badges freizuschalten.',
  needProfileCta: 'Profil erstellen',
  // Navigation cards
  navSectionTitle: 'Bereiche entdecken',
  navSoloTitle: 'Singen',
  navSoloDesc: 'Song aus der Bibliothek wählen und loslegen',
  launchSingle: 'Single',
  launchSingleSub: '1 Spieler',
  launchDuell: 'Duell',
  launchDuellSub: '2 Mics · Punkteduell',
  launchDuett: 'Duett',
  launchDuettSub: '2 Stimmen · Duett-Songs',
  navPartyDesc: '{n} Spielmodi für 2–24 Spieler',
  navJukeboxTitle: 'Jukebox',
  navJukeboxDesc: 'Playlist erstellen, Songs anstehen lassen',
  navHighscoresTitle: 'Bestenlisten',
  navHighscoresDesc: 'Highscores und Rekorde',
  navAchievementsTitle: 'Erfolge',
  navAchievementsDesc: 'Badges und Meilensteine',
  navSettingsTitle: 'Einstellungen',
  navSettingsDesc: 'Audio, Mikrofone, Darstellung',
},
common: {
  loading: 'Laden...',
  error: 'Fehler',
  success: 'Erfolg',
  cancel: 'Abbrechen',
  confirm: 'Bestätigen',
  delete: 'Löschen',
  edit: 'Bearbeiten',
  save: 'Speichern',
  back: 'Zurück',
  next: 'Weiter',
  previous: 'Zurück',
  start: 'Start',
  stop: 'Stopp',
  play: 'Abspielen',
  pause: 'Pause',
  reset: 'Zurücksetzen',
  clear: 'Löschen',
  all: 'Alle',
  none: 'Keine',
  yes: 'Ja',
  no: 'Nein',
  ok: 'OK',
  close: 'Schließen',
  search: 'Suchen',
  filter: 'Filter',
  sort: 'Sortieren',
  refresh: 'Aktualisieren',
  avatarAlt: 'Avatar',
  qrCodeAlt: 'QR-Code',
  unknownError: 'Unbekannter Fehler',
  player: 'Spieler',
},
dialogs: {
  partyExitTitle: 'Party-Modus verlassen?',
  partyExitDesc: 'Ein Party-Modus läuft gerade. Wenn du die Seite verlässt, wird dein aktueller Spielfortschritt verloren gehen.',
  stay: 'Zurück bleiben',
  leave: 'Verlassen',
  partyLeaveTitle: 'Party-Modus verlassen?',
  partyLeaveDesc: 'Du bist dabei, den Party-Modus zu verlassen. Dein aktueller Spielfortschritt geht dabei verloren.',
  back: 'Zurück',
  endParty: 'Party-Modus beenden',
  pauseTitle: 'Spiel pausiert',
  pauseDesc: 'Möchtest du das Spiel fortsetzen oder abbrechen?',
  resume: 'Fortsetzen',
  rematch: '🔄 Game wiederholen',
  setWinner: '🏆 Sieger manuell festlegen',
  abort: 'Abbrechen',
},
connectionStatus: {
  micPlayer: 'Mikrofon-Spieler',
  connected: '{n} ist verbunden',
  disconnected: '{n} ist nicht verbunden',
},
offlineBanner: {
  songsPlaylists: '{songs} Songs, {playlists} Playlists lokal gespeichert',
  localData: 'Lokale Daten verfügbar',
  offline: 'Offline — ',
  serverUnreachable: 'Server nicht erreichbar — Leaderboard und Online-Features nicht verfügbar',
},
uploadStatus: {
  uploading: 'Wird zum globalen Leaderboard hochgeladen...',
  verified: 'Verifizierter Score',
  unverified: 'Unverifiziert (ohne Nachweis)',
  verifiedDesc: 'Dieser Score wurde vom Anti-Cheat-System verifiziert',
  unverifiedDesc: 'Für diesen Score wurde kein Anti-Cheat-Nachweis eingeschlossen',
},
shareSection: {
  title: '📤 Ergebnis teilen',
  scoreCard: '📸 Punktekarte',
  videoShort: '🎬 Video-Short',
  textCopied: 'Punktetext kopiert!',
  textCopyFailed: 'Kopieren fehlgeschlagen',
  imageCopied: 'Punktekarte kopiert!',
  imageCopyFailed: 'Bild kopieren fehlgeschlagen',
  sharingNotSupported: 'Teilen nicht unterstützt. Karte wurde stattdessen heruntergeladen.',
  copyText: '📋 Text kopieren',
  copyImage: '🖼️ Bild kopieren',
  downloadCard: '📥 Karte herunterladen',
  shareScore: '📤 Ergebnis teilen',
},
replayModal: {
  copyrightNotice: 'Hinweis: Aus Urheberrechtsgründen dürfen in exportierten Replays kein originales Audio oder Gesang enthalten sein. Es wird nur deine Mikrofon-Aufnahme verwendet.',
  exportFailed: 'Export fehlgeschlagen. Bitte versuche es erneut.',
  deleteFailed: 'Löschen fehlgeschlagen. Bitte versuche es erneut.',
  close: 'Schließen',
  audioOnly: 'Nur-Audio-Replay',
  originalSong: '🎵 Original-Song',
  export: 'Export…',
  exportShort: '⬇ Export',
  delete: '🗑 Löschen',
  volume: '🎵 Lautstärke',
  replay: 'Replay',
},
scoreCardSocial: {
  branding: 'Karaoke ZERO',
  accuracyLabel: 'Genauigkeit:',
  maxComboLabel: 'Beste Combo:',
  difficultyLabel: 'Schwierigkeit:',
  accuracy: 'Genauigkeit',
  maxCombo: 'Max Combo',
  difficulty: 'Schwierigkeit',
  playerLabel: '🎤 {name}',
  hashtags: '#KaraokeZERO #Karaoke #Singen',
  shareTitle: 'Mein Karaoke-Ergebnis!',
  shareText: 'Ich habe {n} Punkte bei "{title}" von {artist} erreicht!',
  points: 'Punkte',
  download: '📥 Herunterladen',
  share: '📤 Teilen',
},
queue: {
  title: 'Song-Warteschlange',
  empty: 'Warteschlange ist leer',
  emptyDesc: 'Füge Songs aus der Bibliothek hinzu',
  removeFromQueue: 'Aus Warteschlange entfernen',
  clearQueue: 'Warteschlange leeren',
  upNext: 'Als nächstes',
  nowPlaying: 'Jetzt läuft',
  startQueue: 'Warteschlange starten',
},
queueScreen: {
  title: 'Song-Warteschlange',
  songsInQueue: 'Songs in der Warteschlange • Max. 3 pro Spieler',
  noSongs: 'Keine Songs in der Warteschlange',
  noSongsDesc: 'Füge Songs aus der Bibliothek oder über die Companion-App hinzu',
  duel: '⚔️ Duell',
  duet: '🎭 Duett',
  single: '🎤 Solo',
  playerDeactivated: 'Spieler deaktiviert',
  play: '▶ Abspielen',
  clearAll: 'Alle löschen',
  playNextSong: '▶ Nächsten Song spielen',
  playerReselectNeeded: '⚠ Spieler-Neuauswahl benötigt',
  playerReselectDesc: 'Ein Spieler für <strong>{song}</strong> ({mode}) wurde deaktiviert. Bitte wähle neue Spieler oder lösche den Song.',
  assignPlayers: '✓ Spieler zuweisen',
  deleteSong: '✕ Song löschen',
  later: 'Später',
  notEnoughPlayers: 'Nicht genügend aktive Spieler verfügbar (mindestens 2 benötigt).',
  deleteFromQueue: '✕ Song aus Warteschlange löschen',
  rules: 'Warteschlangen-Regeln',
  rule1: '• Maximal 3 Songs pro Spieler gleichzeitig',
  rule2: '• Songs werden in der Reihenfolge abgespielt, in der sie hinzugefügt wurden',
  rule3: '• Du kannst deine eigenen Songs aus der Warteschlange entfernen',
  rule4: '• Wähle einen Charakter vor dem Hinzufügen',
  rule5: '• Companion-App-Anfragen erscheinen mit einem Cyan-Rand',
  rule6: '• Klicke auf einen Song, um ihn sofort abzuspielen',
  companion: '📱 Companion',
},
queueNextSong: {
  label: 'Als nächstes',
  duel: '⚔️ Duell',
  duet: '🎭 Duett',
  playNext: '▶ Nächsten Song spielen',
},
jukebox: {
  title: 'Jukebox-Modus',
  subtitle: 'Setz dich zurück und genieß die Musik!',
  songsInPlaylist: 'Songs in Playlist',
  searchPlaceholder: 'Songs, Künstler, Alben suchen...',
  allGenres: 'Alle Genres',
  allArtists: 'Alle Künstler',
  songsFound: 'Songs gefunden',
  startJukebox: 'Jukebox starten',
  stopJukebox: 'Jukebox stoppen',
  nowPlaying: 'JETZT LÄUFT',
  upNext: 'Als nächstes',
  exitFullscreen: 'Vollbild beenden',
  hidePlaylist: 'Playlist ausblenden',
  showPlaylist: 'Playlist anzeigen',
  playlistSettings: 'Playlist-Einstellungen',
  customizeExperience: 'Passe dein Musik-Erlebnis an',
  filterByGenre: 'Nach Genre filtern',
  filterByArtist: 'Nach Künstler filtern',
  shuffle: 'Zufällig',
  repeat: 'Wiederholen',
  repeatNone: 'Nicht wiederholen',
  repeatAll: 'Alle wiederholen',
  repeatOne: 'Einen wiederholen',
},
jukeboxPlayer: {
  nowPlaying: 'JETZT LÄUFT',
  lyrics: '🎤 Songtext',
  showPlaylist: '📖 Playlist anzeigen',
  hidePlaylist: '📖 Playlist ausblenden',
  exitFullscreen: '⤓ Vollbild beenden',
  stopJukebox: 'Jukebox stoppen',
  upNext: 'Als nächstes',
  jukeboxMode: '🎵 Jukebox',
  sitBack: 'Setz dich zurück und genieße die Musik!',
  sitBackEnjoy: 'Setz dich zurück und genieße die Musik!',
  removeYoutubeVideo: 'YouTube-Video entfernen',
  youtube: 'YouTube',
  fullscreen: '⤢ Vollbild',
  singAlong: 'Sing-Along-Modus: Songtext anzeigen',
  singAlongMode: 'Sing-Along-Modus',
  invalidYoutubeUrl: 'Ungültige YouTube URL',
  youtubeBackground: 'Optionales YouTube-Video als Hintergrund',
  activate: 'Setzen',
  active: 'Aktiv:',
  remove: 'Entfernen',
  noMatchingSongs: 'Keine Songs entsprechen deinen Filtern. Versuche andere Einstellungen oder importiere Songs.',
  searchPlaceholder: 'Songs suchen...',
  allGenres: 'Alle Genres',
  allArtists: 'Alle Künstler',
  filterByGenre: 'Nach Genre filtern',
  filterByArtist: 'Nach Künstler filtern',
  filterByEra: 'Nach Ära filtern',
  allEras: 'Alle Äras',
  shuffle: 'Zufallswiedergabe',
  noRepeat: 'Nicht wiederholen',
  repeatAll: 'Alle wiederholen',
  repeatOne: 'Einen wiederholen',
  songsFound: '{n} Songs gefunden',
  songsAvailable: 'Songs verfügbar',
  startJukebox: 'Jukebox starten',
  playlistSettings: 'Playlist-Einstellungen',
  customizeExperience: 'Passe dein Hörerlebnis an',
  youtubeVideo: 'YouTube-Video',
  youtubeVideoDesc: 'Spiele ein benutzerdefiniertes YouTube-Video als Hintergrund für den aktuellen Song',
  youtubeUrlPlaceholder: 'YouTube-URL hier einfügen...',
  set: 'Setzen',
  activeLabel: 'Aktives Video: {id}',
  noSongsMatchFilters: 'Keine Songs entsprechen deinen Filtern. Versuche andere Einstellungen oder importiere Songs.',
  playlist: 'Playlist',
  allSongs: 'Alle Songs',
  selectPlaylist: 'Playlist auswählen um Songs zu filtern',
  songsInPlaylist: '{n} Songs in der Playlist',
  youtubeRemove: 'YouTube',
  lyricsToggle: 'Liedtext',
  lyricsShort: 'Text',
  songsPlayed: 'gespielt',
  remaining: 'übrig',
  requestedBy: 'Gewünscht von {name}',
  maxSongs: 'Max. Songs',
  unlimited: 'unbegrenzt',
  minutes: 'Min.',
  noTimer: 'aus',
  autoStopTimer: 'Auto-Stop Timer',
  minDuration: 'Min. Dauer',
  maxDuration: 'Max. Dauer',
  recentlyPlayedExclude: 'Kürzlich gespielt ausschließen',
  noExclusion: 'kein Ausschluss',
  genresAvailable: 'Genres',
  artistsAvailable: 'Künstler',
  vinylLabel: 'Rotierende Schallplatte',
  equalizerLabel: 'Audio-Visualisierung',
  heroTagline: 'Musik pur – ohne Punkte, ohne Druck. Einfach genießen.',
  youtubeShort: 'YouTube-Link',
  playbackOptions: 'Wiedergabe-Optionen',
  addYoutubeHint: 'Link aus YouTube kopieren und hier einfügen',
  startReady: 'Bereit zum Abspielen',
  // ── Video-Link / Video-Break-Warteschlange (mit Ton) ──
  videoLinkTitle: 'Video-Link',
  videoLinkDesc: 'Link einreihen — läuft mit Musik und Video, auch ohne Songtext',
  videoLinkPlaceholder: 'Video-Link oder Embed-Code einfügen (YouTube, Dailymotion, Vimeo, Rutube, VK, Bilibili, Niconico, MP4…)',
  videoLinkAdd: 'Einreihen',
  videoLinkInvalid: 'Unerkannter Video-Link — wird noch nicht unterstützt',
  videoLinkQueued: 'Video eingereiht!',
  videoLinkHint: 'Läuft nach dem aktuellen Song bzw. hinter den letzten Wunschsongs — mit Musik und Video. Läuft nichts, startet es sofort. Komplette iframe-Einbettungs-Codes (z. B. VK-Video „Einbetten“) werden direkt akzeptiert.',
  videoQueuedCount: '{n} Videos in der Warteschlange',
  queueEmpty: 'Warteschlange leer — Video-Link oben einreihen oder Songs wünschen',
  videoChip: 'Video',
  queueVideoRemove: 'Video aus der Warteschlange entfernen',
  gateStart: '▶️ Musik/Video läuft — Los!',
  startVideo: 'Video starten',
  startVideoHint: 'Falls das Video nicht von selbst startet — mit einem Klick neu starten',
  volumeInPlayerHint: 'Bilibili bietet keine Player-API — die Lautstärke direkt im Videoplayer regeln',
  // ── Link-Liste ──
  linkListTitle: 'Link-Liste',
  linkListDesc: 'Mehrere Links — einer pro Zeile — werden nacheinander abgespielt',
  linkListPlaceholder: 'https://… (ein Link pro Zeile)\nOptional mit Titel: https://… | Mein Titel',
  linkListAdd: 'Liste einreihen',
  linkListFile: 'Datei',
  linkListParsed: '{n} Links erkannt',
  linkListQueued: '{n} Videos eingereiht!',
  // ── Library-Playlist direkt in der Jukebox ──
  enqueuePlaylist: 'Playlist abspielen',
  enqueuePlaylistDesc: 'Spielt genau diese Playlist in der Jukebox — oder reiht sie hinten ein, wenn bereits etwas läuft',
  enqueuePlaylistRunning: 'In Warteschlange',
  playlistQueued: 'Playlist eingeordnet!',
  // ── Playlist-Browser (Playlists ansehen + zur Jukebox hinzufügen) ──
  browsePlaylists: 'Playlists ansehen',
  browsePlaylistsTitle: '📋 Playlists',
  browsePlaylistsEmpty: 'Noch keine Playlists vorhanden — erstelle eine in der Bibliothek.',
  browsePlaylistsSongs: '{count} Songs',
  browsePlaylistsSetPool: 'Als Pool setzen',
  browsePlaylistsQueue: 'Zur Jukebox hinzufügen',
  browsePlaylistsQueuedToast: 'Playlist zur Jukebox hinzugefügt!',

  // ── Jahres-Filter ──
  filterByYear: 'Nach Jahr filtern',
  allYears: 'Alle Jahre',
  // ── Such-Vorschläge (Fuzzy-Ranking) ──
  searchSuggestions: 'Vorschläge',
  searchNoMatches: 'Keine Treffer',
  addToPlaylist: 'Zur Playlist',
  addAllToPlaylist: 'Alle hinzufügen ({n})',
  addedToPlaylist: 'Zur Playlist hinzugefügt!',
  addedNToPlaylist: '{n} Songs zur Playlist hinzugefügt!',
  alreadyInPlaylist: 'Bereits in der Playlist',
  // ── Allround-Feld für Video-Links (einer oder viele) ──
  videoLinksTitle: 'Video-Links',
  videoLinksDesc: 'Ein oder viele Links — einer pro Zeile — werden nacheinander abgespielt',
  videoLinksPlaceholder: 'Ein Link pro Zeile — optional mit Titel: https://… | Mein Titel',
  videoLinksAdd: 'Hinzufügen',
  videoLinksSupported: 'Alle großen Video-Plattformen werden unterstützt:',
  videoLinksSimpleHint: 'Einfach den Link einfügen',
  videoLinksVkNote: '(für VK Video wird der Embed-Code benötigt)',
  videoLinksFileTypes: 'Unterstützte Dateien: TXT, CSV, TSV, M3U/M3U8, MD, LOG',
},
songPool: 'Song-Pool',
songPoolSelect: 'Wähle einen Song-Pool zum Filtern',
switchPool: 'Pool wechseln',
jukeboxA11y: {
  songProgress: 'Songfortschritt',
  unmute: 'Stumm aus',
  mute: 'Stumm',
  volume: 'Lautstärke',
  shuffle: 'Zufallswiedergabe',
  repeat: 'Wiederholung',
},
fullscreen: {
  exitF12: 'Vollbild beenden (F12)',
  toggleF12: 'Vollbild umschalten (F12)',
  exitEsc: 'Vollbild beenden (ESC)',
  enter: 'Vollbild',
},
errorBoundary: {
  title: 'Etwas ist schiefgelaufen',
  description: 'Ein unerwarteter Fehler ist aufgetreten. Versuche, die Seite neu zu laden.',
  tryAgain: 'Erneut versuchen',
},
countries: {
  germany: 'Deutschland',
  austria: 'Österreich',
  switzerland: 'Schweiz',
  unitedStates: 'Vereinigte Staaten',
  unitedKingdom: 'Vereinigtes Königreich',
  france: 'Frankreich',
  italy: 'Italien',
  spain: 'Spanien',
  netherlands: 'Niederlande',
  poland: 'Polen',
  japan: 'Japan',
  southKorea: 'Südkorea',
  australia: 'Australien',
  canada: 'Kanada',
  brazil: 'Brasilien',
  mexico: 'Mexiko',
},
dailyChallenge: {
  badges: {
    firstChallenge: {
      name: 'Erste Schritte',
      description: 'Schließe deine erste Daily Challenge ab',
    },
    weekWarrior: {
      name: 'Wochen-Krieger',
      description: 'Halte eine 7-Tage-Streak',
    },
    fortnightFighter: {
      name: 'Zweiwochen-Kämpfer',
      description: 'Halte eine 14-Tage-Streak',
    },
    monthlyMaster: {
      name: 'Monats-Meister',
      description: 'Halte eine 30-Tage-Streak',
    },
    podiumFinish: {
      name: 'Podiumsplatz',
      description: 'Beende eine Daily Challenge unter den Top 3',
    },
    dailyChampion: {
      name: 'Tages-Champion',
      description: 'Gewinne eine Daily Challenge',
    },
    dedicatedSinger: {
      name: 'Hingebungsvoller Sänger',
      description: 'Schließe 30 Daily Challenges ab',
    },
    legendaryStatus: {
      name: 'Legendärer Status',
      description: 'Erreiche 10.000 Gesamt-XP',
    },
    centuryChampion: {
      name: 'Jahrhundert-Champion',
      description: 'Halte eine 100-Tage-Streak',
    },
    yearlyLegend: {
      name: 'Jahres-Legende',
      description: 'Halte eine 365-Tage-Streak',
    },
      dailyBronze: { name: 'Daily-Bronze', description: 'Schließe 1 Daily Challenge an einem Tag ab' },
      dailySilver: { name: 'Daily-Silber', description: 'Schließe 3 Daily Challenges an einem Tag ab' },
      dailyGold: { name: 'Daily-Gold', description: 'Schließe alle 5 Daily Challenges an einem Tag ab' },
      weeklyBronze: { name: 'Weekly-Bronze', description: 'Schließe 1 Weekly Challenge in einer Woche ab' },
      weeklySilver: { name: 'Weekly-Silber', description: 'Schließe 3 Weekly Challenges in einer Woche ab' },
      weeklyGold: { name: 'Weekly-Gold', description: 'Schließe alle 5 Weekly Challenges in einer Woche ab' },
  },
  quests: {
    dailyDouble: {
      name: 'Tägliches Doppel',
      description: 'Schließe heute 2 Daily Challenges ab',
    },
    perfectTen: {
      name: 'Perfekte Zehn',
      description: 'Erziele insgesamt 10 perfekte Noten',
    },
    challengeExplorer: {
      name: 'Challenge-Entdecker',
      description: 'Spiele 5 verschiedene Challenge-Modi',
    },
    songbird: {
      name: 'Nachtigall',
      description: 'Beende insgesamt 10 Lieder',
    },
    weeklyWarrior: {
      name: 'Wochen-Krieger',
      description: 'Schließe 3 Wochen-Challenges ab',
    },
  },
  streakMilestones: {
    weekWarrior: 'Wochen-Krieger',
    fortnightFighter: 'Zweiwochen-Kämpfer',
    monthlyMaster: 'Monats-Meister',
    biMonthlyBoss: 'Zwei-Monate-Boss',
    centuryChampion: 'Jahrhundert-Champion',
    yearlyLegend: 'Jahres-Legende',
  },
  // --- Daily Badges (Grid-Anzeige, Kebab-Case-IDs) ---
  dailyBadges: {
    'first-challenge': { name: 'Erste Schritte', description: 'Schließe deine erste Daily Challenge ab' },
    'week-warrior': { name: 'Wochen-Krieger', description: 'Halte eine 7-Tage-Serie' },
    'fortnight-fighter': { name: 'Vierzehn-Tage-Kämpfer', description: 'Halte eine 14-Tage-Serie' },
    'monthly-master': { name: 'Monats-Meister', description: 'Halte eine 30-Tage-Serie' },
    'top-3': { name: 'Podiumsplatz', description: 'Beende eine Daily Challenge in den Top 3' },
    'champion': { name: 'Daily-Champion', description: 'Gewinne eine Daily Challenge' },
    'dedicated': { name: 'Fleißiger Sänger', description: 'Schließe 30 Daily Challenges ab' },
    'legendary': { name: 'Legendärer Status', description: 'Erreiche 10.000 Gesamt-XP' },
    'century-champion': { name: 'Jahrhundert-Champion', description: 'Halte eine 100-Tage-Serie' },
    'yearly-legend': { name: 'Jahres-Legende', description: 'Halte eine 365-Tage-Serie' },
    'daily-bronze': { name: 'Daily-Bronze', description: 'Schließe 1 Daily Challenge an einem Tag ab' },
    'daily-silver': { name: 'Daily-Silber', description: 'Schließe 3 Daily Challenges an einem Tag ab' },
    'daily-gold': { name: 'Daily-Gold', description: 'Schließe alle 5 Daily Challenges an einem Tag ab' },
    'weekly-bronze': { name: 'Weekly-Bronze', description: 'Schließe 1 Weekly Challenge in einer Woche ab' },
    'weekly-silver': { name: 'Weekly-Silber', description: 'Schließe 3 Weekly Challenges in einer Woche ab' },
    'weekly-gold': { name: 'Weekly-Gold', description: 'Schließe alle 5 Weekly Challenges in einer Woche ab' },
    'explorer': { name: 'Challenge-Entdecker', description: 'Spiele 5 verschiedene Challenge-Modi' },
    'songbird': { name: 'Singvogel', description: 'Schließe 10 Songs insgesamt ab' },
    'weekly-warrior': { name: 'Wochen-Krieger', description: 'Schließe 3 Weekly Challenges ab' },
  },

},
desktopChat: {
  title: 'Chat',
  host: 'Host',
  notificationNew: 'Neue Nachricht von {name}',
  openChat: 'Chat öffnen',
  closeChat: 'Chat schließen',
  noMessages: 'Noch keine Nachrichten',
  sendAs: 'Senden als',
  noPlayers: 'Keine Player',
  selectPlayerFirst: 'Zuerst einen Player auswählen',
  placeholder: 'Nachricht eingeben...',
  send: 'Senden',
  challenge: 'Herausfordern',
  challengeSong: 'Herausforderung',
  waitingForOpponent: 'Warte auf Gegner...',
},
songChallenge: {
  challengeBtn: 'Herausfordern',
  acceptBtn: 'Herausforderung annehmen',
  acceptedBy: 'Angenommen von',
  cannotAcceptOwn: 'Du kannst deine eigene Herausforderung nicht annehmen',
},
difficulty: {
  easy: 'Leicht',
  medium: 'Mittel',
  hard: 'Schwer',
},
};
