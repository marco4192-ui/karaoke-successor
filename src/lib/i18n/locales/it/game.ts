// IT translations — game

export const gameTranslations = {
share: {
  points: 'PUNTI',
  accuracy: 'Precisione',
  maxCombo: 'Combo Max',
  rating: 'Voto',
  playerLabel: 'Giocatore: {name}',
  shareTitle: 'Punteggio Karaoke ZERO',
  mode: 'Modalità',
  difficulty: 'Difficoltà',
  scoredPoints: 'Ho appena ottenuto {score} punti con "{title}" di {artist}!',
  callToAction: 'Gioca a {branding} e cerca di battere il mio punteggio!',
},
scoreEvents: {
  perfect: 'PERFETTO',
  great: 'GRANDIOSO',
  good: 'BUONO',
  okay: 'OK',
  miss: 'MANCATO',
},
difficulty: {
  easy: 'Facile',
  medium: 'Medio',
  hard: 'Difficile',
},
game: {
  noteLane: {
    singHere: 'CANTA QUI',
  },
  back: 'Indietro',
  sync: 'Sync',
  pts: 'ptt',
  combo: 'combo',
  practiceMode: 'Modalità Pratica',
  enablePractice: 'Attiva Modalità Pratica',
  playbackSpeed: 'Velocità Riproduzione',
  pitchGuide: 'Guida Tono',
  audioEffects: 'Effetti Audio',
  reverb: 'Riverbero',
  echo: 'Eco',
  pause: 'Pausa',
  resume: 'Riprendi',
  restart: 'Riavvia',
  endSong: 'Termina Canzone',
  lyrics: 'Testo',
  notes: 'Note',
  score: 'Punteggio',
  player: 'Giocatore',
  go: 'VIA!',
  blindWarningIncoming: 'Sezione alla cieca tra {n}s',
  blindWarningHint: 'Le note spariranno — canta a memoria',
  blindWarningActive: 'Sezione alla cieca',
  mwWarningIncoming: 'Parole nascoste tra {n}s',
  mwWarningHint: 'Alcuni testi stanno per sparire',
  mwWarningActive: 'Parole nascoste',
},
gameScreen: {
  noSongSelected: 'Nessuna canzone selezionata',
  noSongLoaded: 'Nessuna canzone caricata',
  backToLibrary: 'Torna alla Libreria',
  pause: '⏸ Pausa',
  lowPerf: '⚡ Prestazioni Ridotte',
  youtubeErrorDeleted: 'Video YouTube non trovato (eliminato o privato)',
  youtubeErrorEmbed: 'Questo video non può essere incorporato (incorporamento disabilitato)',
  youtubeErrorVevo: 'Questo video non può essere incorporato (restrizione Vevo/incorporamento)',
  youtubeErrorInvalid: 'Parametro YouTube non valido',
  youtubeErrorHtml5: 'Errore HTML5 nel player YouTube',
  youtubeErrorCode: 'Errore YouTube (Codice: {n})',
  videoErrorGeo: 'Questo video non è disponibile nella tua regione.',
  videoErrorPlatform: 'Errore video ({platform}, codice {n})',
  loadingMedia: 'Caricamento multimediale...',
  loading: 'Caricamento…',
},
gameHud: {
  audioEffects: 'Effetti Audio',
  reverb: 'Riverbero: {n}%',
  echo: 'Eco: {n}%',
  presets: 'Preset',
  adPlaying: 'Pubblicità in riproduzione',
  gamePaused: 'Gioco in Pausa',
  adPlayingPlatform: 'Pubblicità da {platform}',
  gameStartsAfterAd: 'Il gioco inizia dopo la pubblicità',
  songStartGateTitle: 'In attesa dell\'inizio della canzone',
  songStartGateHint: 'Avvia il video nel player (▶) e conferma quando parte la musica:',
  songStartGateConfirm: 'La musica è partita — via!',
},
gameEnhancements: {
  loadingStats: 'Caricamento statistiche...',
  noCharacter: 'Seleziona o crea un personaggio per vedere le statistiche',
  totalScore: 'Punteggio Totale',
  gamesPlayed: 'Partite Giocate',
  avgAccuracy: 'Precisione Media',
  bestCombo: 'Miglior Combo',
  recentAchievements: 'Obiettivi Recenti',
},
prominentScore: {
  player1: 'Giocatore 1',
  player2: 'Giocatore 2',
  combo: 'COMBO',
  comboMultiplied: '{n}x COMBO!',
},
noteLane: {
  singHere: 'CANTA QUI',
},
practicePanel: {
  title: 'Modalità Pratica',
  enable: 'Attiva Modalità Pratica',
  pitchGuide: 'Guida Tono',
  autoPlayNotes: 'Riproduzione Auto delle Note',
},
mic: {
  title: 'Impostazioni Microfono',
  multiMic: 'Modalità Multi-Microfono',
  multiMicDesc: 'Usa più microfoni contemporaneamente per duetti o canto di gruppo',
  selectDevice: 'Seleziona Microfono',
  defaultMic: 'Microfono Predefinito',
  addMic: 'Aggiungi Microfono',
  removeMic: 'Rimuovi',
  assignToPlayer: 'Assegna a Giocatore',
  level: 'Livello',
  gain: 'Guadagno',
  noiseSuppression: 'Soppressione Rumore',
  noiseSuppressionDesc: 'Riduci il rumore di fondo',
  echoCancellation: 'Cancellazione Eco',
  echoCancellationDesc: 'Riduci il feedback degli altoparlanti',
  test: 'Testa Microfono',
  testing: 'Test in corso...',
  noMicsFound: 'Nessun microfono trovato. Connetti un microfono e aggiorna.',
  connected: 'Connesso',
  disconnected: 'Disconnesso',
},
webcamBackground: {
  enable: 'Attiva Webcam',
  enableDesc: 'Registra i cantanti durante l\'esibizione',
  cameraDevice: 'Dispositivo Fotocamera',
  selectCamera: 'Seleziona fotocamera',
  defaultCamera: 'Fotocamera Predefinita',
  camera: 'Fotocamera',
  size: 'Dimensione',
  position: 'Posizione',
  filter: 'Filtro',
  opacity: 'Opacità',
  showBorder: 'Mostra Bordo',
  fullscreen: 'Schermo Intero',
  top: 'In Alto',
  bottom: 'In Basso',
  left: 'Sinistra',
  right: 'Destra',
  active: 'Attivo',
  errorPermission: 'Permesso fotocamera negato...',
  errorNoCamera: 'Nessuna fotocamera trovata...',
},
results: {
  perfect: 'Perfetto!',
  excellent: 'Eccellente!',
  good: 'Bene!',
  okay: 'Ok!',
  poor: 'Scarso',
  totalScore: 'Punteggio Totale',
  notesHit: 'Note Prese',
  notesMissed: 'Note Mancate',
  bestCombo: 'Miglior Combo',
  accuracy: 'Precisione',
  playAgain: 'Gioca Ancora',
  backToHome: 'Torna alla Home',
  shareScore: 'Condividi Punteggio',
  scoreCard: 'Scheda Punteggio',
  videoShort: 'Video Breve',
  downloadCard: 'Scarica Scheda',
  uploadingToLeaderboard: 'Caricamento nella classifica globale...',
  newHighscore: 'Nuovo Record!',
  rating: 'Valutazione',
  draw: 'Pareggio',
},
resultsScreen: {
  newGlobalHighscore: '🎉 Nuovo record globale!',
  uploadedRank: 'Caricato! Posizione #{n}',
  uploadedRankVerified: 'Uploaded! Rank #{n} (Verified)',
  uploadFailed: 'Caricamento fallito',
  noResults: 'Nessun risultato disponibile',
  accuracyLabel: '{n}% precisione',
  player: 'Giocatore',
  scores: 'Punteggi',
  replay: 'Replay',
},
scoreVisualization: {
  title: 'Analisi del Punteggio',
  poor: 'Scarso',
  okay: 'Ok',
  good: 'Buono',
  excellent: 'Eccellente',
  perfect: 'Perfetto',
  duelComparison: '⚔️ Confronto Duello',
  accuracy: '{n}% precisione',
  maxCombo: '{n}x combo max',
  p1Wins: '🏆 P1 VINCE!',
  p2Wins: '🏆 P2 VINCE!',
  draw: '🤝 PAREGGIO!',
  notesHit: 'Note Centrate',
  maxComboLabel: 'Combo Max',
  player1: 'Giocatore 1',
  player2: 'Giocatore 2',
  score: 'Punteggio',
  combo: 'Combo',
  consistency: 'Coerenza',
  rating: 'Valutazione',
  notes: 'Note',
  finalScore: 'Punteggio Finale',
  maxScore: 'Punteggio Max',
  notesMissed: 'Note Sbagliate',
  category: 'Categoria',
  player1Wins: 'Giocatore 1 Vince!',
  player2Wins: 'Giocatore 2 Vince!',
  drawResult: 'Pareggio!',
  scoreBattle: 'Battaglia di Punteggi',
  player1Abbr: 'P1',
  player2Abbr: 'P2',
  stat: 'Stat',
},
highscore: {
  title: 'Punteggi',
  local: 'Locale',
  global: 'Globale',
  noScores: 'Nessun punteggio ancora',
  noScoresDesc: 'Gioca alcune canzoni per stabilire il tuo primo record!',
  rank: 'Posizione',
  player: 'Giocatore',
  song: 'Canzone',
  score: 'Punteggio',
  date: 'Data',
  clearAll: 'Cancella Tutti i Punteggi',
},
highscoreScreen: {
  title: 'Classifica Record',
  description: 'I migliori cantanti e le loro leggendarie esibizioni!',
  local: '🏠 Locale',
  global: '🌍 Globale',
  allScores: 'Tutti i Punteggi',
  myScores: 'I miei Punteggi',
  rankingTitles: 'Titoli di Classifica',
  loadingGlobal: 'Caricamento classifica globale...',
  retry: '🔄 Riprova',
  switchToLocal: 'Passa a Locale',
  noGlobal: 'Nessun punteggio globale ancora. Sii il primo a caricare!',
  noMine: 'Non hai ancora stabilito nessun punteggio!',
  noAll: 'Nessun record ancora. Sii il primo a cantare!',
  accuracyLabel: '{n}% precisione',
  maxComboLabel: '{n}x combo max',
  totalPoints: 'punti totali',
  searchPlaceholder: 'Search players or songs...',
  searchNoResults: 'No results for "{q}"',
  verified: 'Verified',
  unverified: 'Unverified',
  songsPlayed: '{n} songs',
  bestScore: 'Best: {n}',
  avgAccuracy: '{n}% avg',
  globalRank: 'Global Rank',
  localHighlight: 'Your profile',
  top100: 'Top 100',
  noSearchResults: 'No players found matching your search',
},
keyboardShortcuts: {
  esc: 'Pausa / Indietro / Esci',
  enter: 'Riprendi gioco (Pausa)',
  f12: 'Attiva/Disattiva schermo intero',
  f1f10: 'Navigazione menu',
  ctrlL: 'Cerca nella Libreria',
  ctrlR: 'Canzone Casuale (Singolo)',
  ctrlD: 'Canzone Casuale (Duello)',
  ctrlQ: 'Avvia canzone dalla coda',
  ctrlJ: 'Avvia Jukebox',
  arrows: 'Navigazione',
},
audioAnalysis: {
  confidenceHigh: 'Verde — Affidabile',
  confidenceMedium: 'Giallo — Probabilmente corretto',
  confidenceLow: 'Arancione — Incerto',
  confidenceVeryLow: 'Rosso — Controllo manuale consigliato',
},
battleRoyaleGame: {
  survived: 'SOPRAVVISSUTO!',
},
remoteControl: {
  skipAdTitle: '⏭️ Salta Pubblicità',
  skipAdDesc: 'Clicca sul video per premere il pulsante "Salta Pubblicità"!',
  acquireControl: 'Prendi il controllo',
},
mobilePage: {
  loadingCompanion: 'Caricamento app compagna…',
  failedToLoad: 'Caricamento app compagna fallito',
  retry: 'Riprova',
  unknownError: 'Si è verificato un errore sconosciuto',
},
youtube: {
  errorInvalidParam: 'Il parametro della richiesta contiene un valore non valido.',
  errorHtml5: 'Si è verificato un errore specifico di HTML5.',
  errorVideoNotFound: 'Il video non è stato trovato. Potrebbe essere stato eliminato o contrassegnato come privato.',
  errorEmbed: 'Il video non può essere incorporato.',
  errorUnknown: 'Errore YouTube sconosciuto (Codice: {n})',
},
editor: {
  noSongsFolder: 'Nessuna cartella canzoni configurata. Esegui prima una scansione.',
  saveCancelled: 'Salvataggio annullato',
  noFilePath: 'Nessun percorso file determinato',
  fileSaved: 'File salvato!',
  saveError: 'Errore durante il salvataggio: {error}',
  saveDialogTitle: 'Salva file TXT',
  saveDialogFilter: 'UltraStar TXT',
  networkError: 'Errore di rete',
},
paused: 'IN PAUSA',
go: 'VIA!',
common: {
  unknown: 'Sconosciuto',
  unknownArtist: 'Artista sconosciuto',
  defaultPlayerName: 'Giocatore',
},
page: {
  loading: 'Caricamento di Karaoke ZERO...',
  reload: 'Ricarica',
},
achievements: {
  firstNote: {
    name: 'Prima Nota',
    description: 'Colpisci la tua prima nota',
  },
  perfectTen: {
    name: 'Dieci Perfetti',
    description: 'Ottieni 10 colpi Perfetti in una singola canzone',
  },
  comboMaster: {
    name: 'Maestro del Combo',
    description: 'Raggiungi un combo di 50 note',
  },
  comboKing: {
    name: 'Re del Combo',
    description: 'Raggiungi un combo di 100 note',
    rewardTitle: 'Re del Combo',
  },
  comboLegend: {
    name: 'Leggenda del Combo',
    description: 'Raggiungi un combo di 200 note',
    rewardTitle: 'Leggenda del Combo',
  },
  perfectSong: {
    name: 'Canzone Perfetta',
    description: 'Ottieni 99.5%+ di precisione su una canzone',
    rewardTitle: 'Perfezionista',
  },
  pitchPerfect: {
    name: 'Tono Perfetto',
    description: 'Ottieni oltre il 90% di precisione',
  },
  risingStar: {
    name: 'Stella Nascente',
    description: 'Segna più di 8.000 punti',
  },
  scoreMaster: {
    name: 'Maestro del Punteggio',
    description: 'Segna più di 9.000 punti',
  },
  flawless: {
    name: 'Impeccabile',
    description: 'Segna più di 9.500 punti',
    rewardTitle: 'Impeccabile',
  },
  goldenCollector: {
    name: 'Collezionista Dorato',
    description: 'Colpisci 10 note dorate',
  },
  goldenMaster: {
    name: 'Maestro Dorato',
    description: 'Colpisci 50 note dorate',
    rewardTitle: 'Voce Dorata',
  },
  firstSong: {
    name: 'Primi Passi',
    description: 'Completa la tua prima canzone',
  },
  tenSongs: {
    name: 'Appassionato di Karaoke',
    description: 'Completa 10 canzoni',
  },
  fiftySongs: {
    name: 'Cantante Regolare',
    description: 'Completa 50 canzoni',
  },
  hundredSongs: {
    name: 'Leggenda del Karaoke',
    description: 'Completa 100 canzoni',
    rewardTitle: 'Leggenda del Karaoke',
  },
  fiveGames: {
    name: 'Primi Passi',
    description: 'Gioca 5 partite',
  },
  twentyGames: {
    name: 'Cantante Dedicato',
    description: 'Gioca 20 partite',
  },
  partyTime: {
    name: 'Ora di Festa!',
    description: 'Gioca una modalità gioco festa',
  },
  duelChampion: {
    name: 'Campione del Duello',
    description: 'Vinci un incontro di duello',
  },
  passTheMic: {
    name: 'Passa il Microfono!',
    description: 'Gioca la modalità Passa il Microfono',
  },
  showerSinger: {
    name: 'Cantante della Doccia',
    description: 'Ottieni meno del 20% di precisione su una canzone',
    rewardTitle: 'Cantante della Doccia',
  },
  comebackKing: {
    name: 'Re del Ritorno',
    description: 'Raggiungi un combo di 50+ dopo aver sbagliato 10 note',
  },
  speedDemon: {
    name: 'Demone della Velocità',
    description: 'Completa una canzone a velocità 1.5x',
  },
  blindMaster: {
    name: 'Maestro Cieco',
    description: 'Completa una canzone in modalità Karaoke Cieco',
    rewardTitle: 'Maestro Cieco',
  },
  dailyStarter: {
    name: 'Esordiente Quotidiano',
    description: 'Completa la tua prima sfida quotidiana',
  },
  dailyRegular: {
    name: 'Frequentatore Quotidiano',
    description: 'Completa 10 sfide quotidiane',
  },
  dailyDevoted: {
    name: 'Devoto Quotidiano',
    description: 'Completa 50 sfide quotidiane',
  },
  streakWeek: {
    name: 'In Fiamme',
    description: 'Mantieni una serie quotidiana di 7 giorni',
  },
  streakMonth: {
    name: 'Inarrestabile',
    description: 'Mantieni una serie quotidiana di 30 giorni',
    rewardTitle: 'Inarrestabile',
  },
  weeklyWarrior: {
    name: 'Guerriero Settimanale',
    description: 'Completa 5 sfide settimanali',
  },
  precisionSinger: {
    name: 'Cantante di Precisione',
    description: 'Ottieni oltre il 95% di precisione',
  },
  goldenRush: {
    name: 'Febbre dell\'Oro',
    description: 'Colpisci 20 note dorate in una sola canzone',
  },
  goldenCenturion: {
    name: 'Centurione Dorato',
    description: 'Colpisci 100 note dorate in totale',
  },
  perfectFifty: {
    name: 'Cinquanta Perfette',
    description: 'Colpisci 50 note perfette in una sola canzone',
  },
  lightningLips: {
    name: 'Labbra Fulminee',
    description: 'Completa una canzone a velocità 2x',
  },
  perfectHarmony: {
    name: 'Armonia Perfetta',
    description: 'Canta 10 duetti',
  },
  genreExplorer: {
    name: 'Esploratore di Generi',
    description: 'Canta canzoni di 5 generi diversi',
  },
  disneyFan: {
    name: 'Fan di Disney',
    description: 'Canta 10 canzoni Disney',
  },
  nightOwl: {
    name: 'Gufo Notturno',
    description: 'Termina una canzone tra mezzanotte e le 4 del mattino',
  },
  earlyBird: {
    name: 'Mattiniero',
    description: 'Termina una canzone prima delle 8 del mattino',
  },
  marathonSinger: {
    name: 'Cantante Maratoneta',
    description: 'Gioca 5 partite in un solo giorno',
  },

  // ── Espansione a 100 obiettivi ──
  score9800: {
    name: 'Stella Ultra',
    description: 'Segna più di 9.800 punti',
  },
  score9900: {
    name: 'Oltre la Perfezione',
    description: 'Segna più di 9.900 punti',
    rewardTitle: 'Oltre la Perfezione',
  },
  combo300: {
    name: 'Titano della Combo',
    description: 'Raggiungi una combo di 300 note',
  },
  combo500: {
    name: 'Combo Immortale',
    description: 'Raggiungi una combo di 500 note',
    rewardTitle: 'Combo Immortale',
  },
  accuracy92: {
    name: 'Messa a Punto',
    description: 'Ottieni oltre il 92% di precisione',
  },
  accuracy94: {
    name: 'Qualità da Studio',
    description: 'Ottieni oltre il 94% di precisione',
  },
  accuracy96: {
    name: 'Tiratore Scelto',
    description: 'Ottieni oltre il 96% di precisione',
  },
  accuracy97: {
    name: 'Precisione Laser',
    description: 'Ottieni oltre il 97% di precisione',
  },
  accuracy98: {
    name: 'Virtuoso',
    description: 'Ottieni oltre il 98% di precisione',
  },
  perfect75: {
    name: 'Settantacinque Perfette',
    description: 'Colpisci 75 note perfette in una sola canzone',
  },
  perfect100: {
    name: 'Centuria Perfetta',
    description: 'Colpisci 100 note perfette in una sola canzone',
  },
  perfect150: {
    name: 'Tempesta Perfetta',
    description: 'Colpisci 150 note perfette in una sola canzone',
  },
  golden30: {
    name: 'Marea Dorata',
    description: 'Colpisci 30 note dorate in una sola canzone',
  },
  golden40: {
    name: 'Sinfonia Dorata',
    description: 'Colpisci 40 note dorate in una sola canzone',
  },
  perfect500: {
    name: 'Macchina Perfetta',
    description: 'Colpisci 500 note perfette in totale',
  },
  perfect1000: {
    name: 'Cannone di Precisione',
    description: 'Colpisci 1.000 note perfette in totale',
  },
  perfect5000: {
    name: 'Valanga Perfetta',
    description: 'Colpisci 5.000 note perfette in totale',
  },
  perfect10000: {
    name: 'Diecimila Perfette',
    description: 'Colpisci 10.000 note perfette in totale',
    rewardTitle: 'Diecimila Perfette',
  },
  golden250: {
    name: 'Raccolto Dorato',
    description: 'Colpisci 250 note dorate in totale',
  },
  golden1000: {
    name: 'Diluvio Dorato',
    description: 'Colpisci 1.000 note dorate in totale',
  },
  golden5000: {
    name: 'Voce di Mida',
    description: 'Colpisci 5.000 note dorate in totale',
    rewardTitle: 'Voce di Mida',
  },
  songs250: {
    name: 'Veterano del Repertorio',
    description: 'Completa 250 canzoni',
  },
  songs500: {
    name: 'Club dei Cinquecento',
    description: 'Completa 500 canzoni',
  },
  songs1000: {
    name: 'Leggenda delle Mille Canzoni',
    description: 'Completa 1.000 canzoni',
    rewardTitle: 'Leggenda delle Mille Canzoni',
  },
  games50: {
    name: 'Cantante Frequente',
    description: 'Gioca 50 partite',
  },
  games100: {
    name: 'Club del Centinaio',
    description: 'Gioca 100 partite',
  },
  games250: {
    name: 'Habitué della Sala Giochi',
    description: 'Gioca 250 partite',
  },
  games500: {
    name: 'Maniaco della Maratona',
    description: 'Gioca 500 partite',
    rewardTitle: 'Maniaco della Maratona',
  },
  level25: {
    name: 'Cantante Esperto',
    description: 'Raggiungi il livello 25',
  },
  level50: {
    name: 'Cantante d\'Élite',
    description: 'Raggiungi il livello 50',
    rewardTitle: 'Cantante d\'Élite',
  },
  level100: {
    name: 'Leggenda di Livello 100',
    description: 'Raggiungi il livello 100',
    rewardTitle: 'Leggenda di Livello 100',
  },
  daily100: {
    name: 'Centurione Quotidiano',
    description: 'Completa 100 sfide quotidiane',
  },
  daily250: {
    name: 'Fanatico Quotidiano',
    description: 'Completa 250 sfide quotidiane',
  },
  daily500: {
    name: 'Immortale Quotidiano',
    description: 'Completa 500 sfide quotidiane',
    rewardTitle: 'Immortale Quotidiano',
  },
  streak60: {
    name: 'Volontà di Ferro',
    description: 'Mantieni una serie quotidiana di 60 giorni',
  },
  streak100: {
    name: 'Eroe dei Cento Giorni',
    description: 'Mantieni una serie quotidiana di 100 giorni',
  },
  streak180: {
    name: 'Devozione di Mezzo Anno',
    description: 'Mantieni una serie quotidiana di 180 giorni',
    rewardTitle: 'Devozione di Mezzo Anno',
  },
  streak365: {
    name: 'Leggenda Annuale',
    description: 'Mantieni una serie quotidiana di 365 giorni',
    rewardTitle: 'Leggenda Annuale',
  },
  weekly15: {
    name: 'Incondizionato Settimanale',
    description: 'Completa 15 sfide settimanali',
  },
  weekly30: {
    name: 'Pilastro Settimanale',
    description: 'Completa 30 sfide settimanali',
  },
  weekly52: {
    name: 'Un Anno di Settimane',
    description: 'Completa 52 sfide settimanali',
    rewardTitle: 'Un Anno di Settimane',
  },
  encore10: {
    name: 'Bis!',
    description: 'Gioca 10 partite in un solo giorno',
  },
  duets25: {
    name: 'Devoto dei Duetti',
    description: 'Canta 25 duetti',
  },
  duets50: {
    name: 'Duo Dinamico',
    description: 'Canta 50 duetti',
  },
  duets100: {
    name: 'Centuria di Duetti',
    description: 'Canta 100 duetti',
    rewardTitle: 'Centuria di Duetti',
  },
  duels5: {
    name: 'Duellante',
    description: 'Vinci 5 duelli',
  },
  duels10: {
    name: 'Maestro del Duello',
    description: 'Vinci 10 duelli',
  },
  duels25: {
    name: 'Signore del Duello',
    description: 'Vinci 25 duelli',
    rewardTitle: 'Signore del Duello',
  },
  party10: {
    name: 'Animale da Festa',
    description: 'Gioca 10 partite in modalità festa',
  },
  party25: {
    name: 'Anima della Festa',
    description: 'Gioca 25 partite in modalità festa',
  },
  party50: {
    name: 'Leggenda della Festa',
    description: 'Gioca 50 partite in modalità festa',
  },
  disney25: {
    name: 'Appassionato di Disney',
    description: 'Canta 25 canzoni Disney',
  },
  disney50: {
    name: 'C\'era una Volta una Canzone',
    description: 'Canta 50 canzoni Disney',
    rewardTitle: 'Famiglia Reale Disney',
  },
  genres8: {
    name: 'Vagabondo dei Generi',
    description: 'Canta canzoni di 8 generi diversi',
  },
  genres10: {
    name: 'Esperto di Generi',
    description: 'Canta canzoni di 10 generi diversi',
  },
  cleanSheet: {
    name: 'Porta Inviolata',
    description: 'Termina una canzone con 50+ note e zero errori',
  },
  weekendSinger: {
    name: 'Cantante del Weekend',
    description: 'Termina una canzone di sabato o domenica',
  },
  lunchBreak: {
    name: 'Pausa Pranzo',
    description: 'Termina una canzone tra le 12 e le 14',
  },
},
ranks: {
  beginner: {
    name: 'Principiante',
    titles: {
      newcomer: 'Novellino',
    },
  },
  novice: {
    name: 'Novizio',
    titles: {
      risingStar: 'Stella Nascente',
    },
  },
  apprentice: {
    name: 'Apprendista',
    titles: {
      melodyMaker: 'Creatore di Melodie',
    },
  },
  singer: {
    name: 'Cantante',
    titles: {
      voiceInTraining: 'Voce in Addestramento',
    },
  },
  performer: {
    name: 'Interprete',
    titles: {
      stagePresence: 'Presenza Scenica',
    },
  },
  artist: {
    name: 'Artista',
    titles: {
      artisticSoul: 'Anima Artistica',
    },
  },
  star: {
    name: 'Stella',
    titles: {
      shiningStar: 'Stella Splendente',
    },
  },
  superstar: {
    name: 'Superstar',
    titles: {
      crowdFavorite: 'Preferito dal Pubblico',
    },
  },
  legend: {
    name: 'Leggenda',
    titles: {
      legendaryVoice: 'Voce Leggendaria',
    },
  },
  icon: {
    name: 'Icona',
    titles: {
      musicalIcon: 'Icona Musicale',
    },
  },
  mythic: {
    name: 'Mitico',
    titles: {
      mythicSinger: 'Cantante Mitico',
    },
  },
  divine: {
    name: 'Divino',
    titles: {
      divineVoice: 'Voce Divina',
    },
  },
},
challenges: {
  blindAudition: {
    name: 'Audizione Cieca',
    description: 'Canta senza vedere il testo — test di memoria!',
  },
  freeFlight: {
    name: 'Volo Libero',
    description: 'Nessuna guida tonale — canta ad orecchio!',
  },
  speedDemon: {
    name: 'Demone della Velocità',
    description: 'Velocità 1.5x — pensa veloce!',
  },
  perfectionist: {
    name: 'Perfezionista',
    description: 'Solo le note perfette contano!',
  },
  goldenHunter: {
    name: 'Cacciatore Dorato',
    description: 'Solo le note dorate danno punti — prendile tutte!',
  },
  memoryLane: {
    name: 'Via della Memoria',
    description: 'Sfida delle parole mancanti — riempi gli spazi vuoti!',
  },
  pitchShift: {
    name: 'Cambio di Tonalità',
    description: 'La canzone è trasportata — adatta la tua voce!',
  },
  halfSpeed: {
    name: 'Rallentatore',
    description: 'Velocità 0.75x — perfetto per esercitarsi!',
  },
  blindMaster: {
    name: 'Maestro Cieco',
    description: 'Nessun testo E nessuna guida tonale — canto davvero cieco!',
  },
  ultimateChallenge: {
    name: 'Sfida Finale',
    description: 'Tutti i modificatori combinati — per i coraggiosi!',
  },
},
modifiers: {
  noLyrics: {
    label: 'Senza Testo',
    description: 'Il testo è nascosto',
    shortDescription: 'Senza testo',
  },
  noPitchGuide: {
    label: 'Senza Guida Tonale',
    description: 'La guida tonale è nascosta',
    shortDescription: 'Senza guida tonale',
  },
  doubleSpeed: {
    label: 'Accelerazione',
    description: 'La canzone suona più veloce',
    shortDescription: 'Velocità 1.25x',
  },
  halfSpeed: {
    label: 'Rallentatore',
    description: 'La canzone suona più lentamente',
  },
  perfectOnly: {
    label: 'Perfezionista',
    description: 'Solo le note perfette contano',
  },
  goldenOnly: {
    label: 'Cacciatore Dorato',
    description: 'Solo le note dorate contano',
  },
  missingWords: {
    label: 'Parole Mancanti',
    description: 'Alcune parole sono nascoste',
  },
  blind: {
    label: 'Cieco',
    description: 'Nessun testo e nessuna guida tonale',
  },
  pitchShift: {
    description: 'Tonalità spostata di 3 semitoni',
  },
},
rankingTitles: {
  showerSingingSensation: 'Sensazione del Canto sotto la Doccia',
  karaokeRoyalty: 'Realeza del Karaoke',
  vocalVirtuoso: 'Virtuoso Vocale',
  micDropMaster: 'Maestro del Mic Drop',
  diamondVoice: 'Voce di Diamante',
  broadwayWannabe: 'Aspirante di Broadway',
  noteNailer: 'Chiodo delle Note',
  pitchyParrot: 'Pappagallo Stonato',
  circusSinger: 'Cantante da Circo',
  humbleHummer: 'Cantante Umile',
  underConstruction: 'In Costruzione',
  bathroomBaritone: 'Baritono del Bagno',
  phantomPhony: 'Fantasma Falso',
  duckTapeSinger: 'Cantante col Nastro Adesivo',
  tunelessTroubadour: 'Trovatore Senza Tono',
  vocalTornado: 'Tornado Vocale (Disastro)',
  toneDeafTitan: 'Titano Sordo',
  clownCarCrooner: 'Crooner di Auto dei Pagliacci',
  toneZombie: 'Zombie Tonale',
  whisperingWimp: 'Sussurratore Timido',
  silentScream: 'Urlo Silenzioso',
},
battleRoyale: {
  bounty: 'TAGLIA',
  micError: 'Micro',
  survived: 'Sopravvissuto',
  visualSettings: 'Impostazioni visive',
},
blind: {
  title: 'Karaoke alla Cieca',
},

missingWords: {
  title: 'Parole Mancanti',
},

competitive: {
  start: 'Inizia',
},

};
