...
import { registerServiceWorker, isPWAInstalled, promptPWAInstall, initPWAInstall } from '@/lib/game/pwa';
...
function getPitchClass(midiNote: number): number {
  return ((midiNote % 12) + 12) % 12; // Ensure positive result
}
 ...
 * Returns { points, hitType, octaveBonus, displayType }
 * 
 * Scoring is designed so that a perfect song yields ~10,000 points
 * - Perfect: 100 base points (green/yellow gradient)
 * - Great: 75 base points (green gradient)  
 * - Good: 50 base points (blue gradient)
 * - Miss: 0 points (red/gray)
 */
...
// Screen types
type Screen = 'home' | 'library' | 'game' | 'party' | 'character' | 'queue' | 'mobile' | 'results' | 'highscores' | 'import' | 'settings' | 'jukebox' | 'achievements' | 'dailyChallenge';
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <NavButton active={screen === 'dailyChallenge'} onClick={() => setScreen('dailyChallenge')}>
              <StarIcon className="w-5 h-5" /> Daily
            </NavButton>
...
type LibraryViewMode = 'grid' | 'folder';
      ...
      {!songsLoading && (
        <div className="space-y-4 mb-6">
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            {(settings.filterGenre !== 'all' || settings.filterLanguage !== 'all') && (
              <button
                onClick={() => setSettings(prev => ({ ...prev, filterGenre: 'all', filterLanguage: 'all' }))}
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <div className="flex bg-white/5 rounded-lg p-1">
              <button
                onClick={() => { setViewMode('grid'); setGroupBy('none'); setCurrentFolder(null); setFolderBreadcrumb([]); }}
...
// Supports different lyrics styles from settings
function LyricLineDisplay({ line, currentTime, playerColor }: { line: LyricLine; currentTime: number; playerColor: string }) {
  ...
  const [lyricsStyle, setLyricsStyle] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('karaoke-lyrics-style') || 'classic';
    }
    return 'classic';
  });
  ...
  return (
    <span className="text-2xl md:text-3xl flex flex-wrap justify-center gap-x-2">
      {line.notes.map((note, idx) => {
        ...
        return (
          <span key={note.id || `note-${idx}`}>
            <span 
              className={`${fontClass} ${textClass} transition-all duration-100`}
              style={shadowStyle}
            >
              {note.lyric}
            </span>
            {/* Add space between words except for the last word */}
            {idx < line.notes.length - 1 && (
              <span className="text-white/30"> </span>
            )}
          </span>
        );
...
function GameScreen({ onEnd, onBack }: { onEnd: () => void; onBack: () => void }) {
  ...
  const [showBackgroundVideo, setShowBackgroundVideo] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('karaoke-bg-video') !== 'false';
    }
    return true;
  });
  const [showPitchGuide, setShowPitchGuide] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('karaoke-show-pitch-guide') !== 'false';
    }
    return true;
  });
  ...
  const beatDuration = song?.bpm ? 15000 / song.bpm : 500; // ms per beat
          ...
          if (currentCount <= 0) {
            ...
            const playMedia = async () => {
              try {
                ...
                // Media playback failed silently
              }
            };
            playMedia();
          } else {
  ...
  // Check if player hits notes - using UltraStar-style relative pitch (octave wrapping)
  const checkNoteHits = useCallback((currentTime: number, pitch: { frequency: number | null; note: number | null; clarity: number; volume: number }) => {
    ...
    // Track if we hit any note this frame (for scoring feedback)
    let hitAnyNote = false;
    
    // Get total notes for score normalization
    const totalNotes = timingData?.noteCount || 100;
    ...
    if (timingData) {
      for (const note of timingData.allNotes) {
        ...
        if (currentTime >= note.startTime && currentTime <= noteEnd) {
          // Use UltraStar-style relative pitch comparison
          ...
          if (result.points > 0) {
            ...
            updatePlayer(activePlayer.id, {
              score: activePlayer.score + result.points,
              combo: newCombo,
              maxCombo: Math.max(activePlayer.maxCombo, newCombo),
              notesHit: activePlayer.notesHit + 1,
            });
            
            // Add visual feedback event with displayType for colored popups
            ...
            // Pitch was wrong - break combo and show Miss
            updatePlayer(activePlayer.id, {
              combo: 0,
  ...
  }, [song, gameState.difficulty, gameState.players, updatePlayer, timingData]);
  ...
  const endGameAndCleanup = useCallback(() => {
    ...
    generateResults();
    onEnd();
  }, [stop, audioEffects, endGame, generateResults, onEnd]);
  ...
  }, [isPlaying, song, pitchResult, setCurrentTime, setDetectedPitch, checkNoteHits, endGameAndCleanup, isYouTube, youtubeTime, timingOffset]);
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-black">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {/* Video Background - only show if setting is enabled */}
        {showBackgroundVideo && isYouTube && youtubeVideoId ? (
          <YouTubePlayer
            ...
            startTime={0}
          />
        ) : showBackgroundVideo && song.videoBackground ? (
          <video
            ref={videoRef}
            src={song.videoBackground}
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20">
          <div className="bg-gradient-to-t from-black/80 to-transparent p-6">
            ...
            {(() => {
              ...
              return (
                <div className="text-2xl md:text-3xl font-bold text-center drop-shadow-lg">
                  <LyricLineDisplay 
                    line={displayLine} 
            ...
            {(() => {
              ...
              return (
                <p className="text-base md:text-lg text-center text-white/40 mt-3">
                  {nextLineText}
                </p>
              );
            })()}
...
function ProfileSyncSection({ profile }: { profile: PlayerProfile }) {
  ...
  const handleUploadProfile = async () => {
    ...
    try {
      ...
      // Update profile with sync code
      updateProfile(profile.id, { syncCode: code });
      
      // In a real implementation, this would call the API
      // await uploadProfile(profile, code);
      ...
      setMessage({ type: 'error', text: 'Failed to upload profile' });
    } finally {
  ...
  const handleDownloadProfile = async () => {
    ...
    try {
      // In a real implementation, this would call the API
      // const downloadedProfile = await downloadProfileByCode(inputCode);
      ...
      setMessage({ type: 'error', text: 'Failed to download profile. Check the sync code.' });
    } finally {
...
type MobileView = 'home' | 'profile' | 'songs' | 'queue' | 'mic';
...
function MobileClientView() {
  ...
  const [gameState, setGameState] = useState<{ currentSong: { title: string; artist: string } | null; isPlaying: boolean }>({ currentSong: null, isPlaying: false });
  ...
  // Queue state
  const [queue, setQueue] = useState<Array<{ songId: string; songTitle: string; songArtist: string; addedBy: string }>>([]);
  ...
  const loadSongs = useCallback(async () => {
    setSongsLoading(true);
    try {
      // Fetch songs from the song library API
  ...
  const startMicrophone = useCallback(async () => {
    ...
    try {
      ...
      const detectPitch = () => {
        ...
        if (clientId && (volume > 0.01 || frequency !== null)) {
          fetch('/api/mobile', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 text-white">
      ...
      {!isConnected ? (
        ...
        <div className="pb-20">
          ...
          {currentView === 'queue' && (
            <div className="p-4">
              <h2 className="text-lg font-semibold mb-4">Song Queue</h2>
    ...
    if (onlineEnabled && leaderboardType === 'global') {
      ...
      import('@/lib/api/leaderboard-service').then(({ leaderboardService }) => {
        leaderboardService.getGlobalLeaderboard(50)
          ...
          .catch(err => {
            setGlobalError(err.message || 'Failed to load global leaderboard');
          })
      ...
      {globalError && (
        <Card className="bg-red-500/10 border-red-500/30 mb-6">
          <CardContent className="py-4 text-center">
            ...
            <Button onClick={() => setLeaderboardType('local')} size="sm" className="mt-2 bg-white/10">
              Switch to Local
            </Button>
          </CardContent>
        </Card>
...
function ResultsScreen({ onPlayAgain, onHome }: { onPlayAgain: () => void; onHome: () => void }) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {/* Score Display */}
      <div className="mb-8">
        <ScoreDisplay player={playerForDisplay} showCombo={true} />
      </div>
...
function SettingsScreen() {
  ...
  const [activeTab, setActiveTab] = useState<'library' | 'general' | 'about'>('library');
  ...
  useEffect(() => {
    ...
    const savedPreviewVolume = localStorage.getItem('karaoke-preview-volume');
    if (savedPreviewVolume) setPreviewVolume(parseInt(savedPreviewVolume));
    
    const savedMicSensitivity = localStorage.getItem('karaoke-mic-sensitivity');
    if (savedMicSensitivity) setMicSensitivity(parseInt(savedMicSensitivity));
    
    const savedDifficulty = localStorage.getItem('karaoke-default-difficulty') as Difficulty;
    if (savedDifficulty) setDefaultDifficulty(savedDifficulty);
    
    const savedPitchGuide = localStorage.getItem('karaoke-show-pitch-guide');
    if (savedPitchGuide !== null) setShowPitchGuide(savedPitchGuide === 'true');
  ...
  // Handle difficulty change - apply globally
  ...
  // Handle pitch guide toggle - apply globally
  const handlePitchGuideToggle = (enabled: boolean) => {
    setShowPitchGuide(enabled);
    localStorage.setItem('karaoke-show-pitch-guide', enabled.toString());
    // Dispatch event for GameScreen to pick up
    window.dispatchEvent(new CustomEvent('settingsChange', { 
      detail: { showPitchGuide: enabled } 
    }));
    forceUpdate(n => n + 1);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex gap-2 mb-6">
        ...
        >
          <MusicIcon className="w-4 h-4 mr-2" /> {tx('settings.tabAbout')}
        </Button>
      </div>
              ...
              <div className="space-y-3">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    localStorage.setItem('karaoke-preview-volume', val.toString());
                  }}
              ...
              <div className="space-y-3">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    localStorage.setItem('karaoke-mic-sensitivity', val.toString());
                  }}
                  className="w-full accent-cyan-500 h-2 rounded-lg appearance-none bg-white/10 cursor-pointer"
                />
      ...
      {activeTab === 'about' && (
        <div className="space-y-6">
          ...
          <div className="mt-4 p-4 bg-white/5 rounded-lg">
            ...
            {isPWAInstalled() && (
              <p className="text-xs text-green-400 mt-2">✓ App is installed</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              >
                <option value="default">{tx('mic.defaultMic')}</option>
              ...
              {assignedMics.map((mic, index) => (
                <div key={mic.id} className="bg-white/5 rounded-lg p-3 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <option value="">-- {tx('mic.assignToPlayer')} --</option>
                      {profiles.map((profile) => (
...
function JukeboxScreen({ onBack }: { onBack: () => void }) {
  ...
  // Auto-play when song changes - FIXED: properly start audio playback
  useEffect(() => {
    if (isPlaying && currentSong) {
      ...
      const playTimer = setTimeout(() => {
        ...
        // Play audio if available (separate audio file or video without embedded audio)
              ...
              <div className={`relative ${isFullscreen ? 'h-full' : 'aspect-video'}`}>
                ...
                {currentSong.videoBackground ? (
                  <video
                    ...
                    className="absolute inset-0 w-full h-full object-cover"
                    muted={!currentSong.hasEmbeddedAudio && !!currentSong.audioUrl}
                    loop={false}
                ...
                {/* Audio element for songs with separate audio */}
                {currentSong.audioUrl && (!currentSong.videoBackground || !currentSong.hasEmbeddedAudio) && (
                  <audio
                    ref={audioRef}
...
function DailyChallengeScreen({ onPlayChallenge }: { onPlayChallenge: (song: Song) => void }) {
  ...
  const getDailyChallenge = () => {
    ...
    const types = ['score', 'accuracy', 'combo', 'songs', 'perfect_notes'];
    const type = types[seed % types.length];
    const targets = { score: 80000, accuracy: 85, combo: 50, songs: 3, perfect_notes: 20 };
    return { type, target: targets[type as keyof typeof targets], seed };
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8 text-center">
        ...
        <p className="text-white/60">Complete daily challenges to build your streak!</p>
      </div>
      
      {/* Streak Banner */}
      <Card className="bg-gradient-to-r from-orange-500/20 to-yellow-500/20 border-orange-500/30 mb-6">
        <CardContent className="pt-6 text-center">
          <div className="text-5xl mb-2">🔥</div>
          <div className="text-3xl font-bold text-orange-400">{streak} Day Streak</div>
          <p className="text-white/60 text-sm mt-1">Keep playing daily to maintain your streak!</p>
        </CardContent>
      </Card>
      
      {/* Challenge Card */}
      <Card className={`bg-white/5 border-white/10 mb-6 ${completed ? 'ring-2 ring-green-500' : ''}`}>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
        ...
        <CardContent>
          <p className="text-lg mb-4">{challengeDescriptions[challenge.type as keyof typeof challengeDescriptions]}</p>
          <div className="flex items-center justify-between">
            <div className="text-sm text-white/60">
            ...
            {!completed && (
              <Button onClick={() => {
                ...
                if (songs.length > 0) onPlayChallenge(songs[0]);
