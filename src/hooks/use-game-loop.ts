...
export function useGameLoop(
    ...
    (detectedPitch: number | null, currentTime: number) => {
      ...
      for (const activeNote of activeNotes) {
        ...
        if (detectedPitch !== null && !processedNotesRef.current.has(activeNote.noteId)) {
          ...
          // Use UltraStar-style relative pitch comparison (octave-wrapped)
          const relativeDiff = getRelativePitchDiff(sungMidi, activeNote.targetPitch);
          
          // For hard mode, require exact pitch class (no tolerance)
          const effectiveTolerance = gameState.difficulty === 'hard' ? 0 : settings.pitchTolerance;
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      // Check if song ended
      if (gameState.currentSong && newTime >= gameState.currentSong.duration) {
        endGame();
        return;
      }
    };
