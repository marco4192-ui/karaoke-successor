...
interface BattleRoyaleRoundSetupProps {
  game: BattleRoyaleGame;
  activeMicPlayers: number;
  activeCompanionPlayers: number;
  activePlayersCount: number;
  onStartRound: () => void;
}

export function BattleRoyaleRoundSetup({
  game,
  activeMicPlayers,
  activeCompanionPlayers,
  activePlayersCount,
  onStartRound,
}: BattleRoyaleRoundSetupProps) {
  return (
    <div className="max-w-5xl mx-auto text-center">
      <h1 className="text-3xl font-bold mb-2">Round {game.currentRound + 1}</h1>
      <p className="text-white/60 mb-6">
        {activeMicPlayers} 🎤 Mic + {activeCompanionPlayers} 📱 Companion = {activePlayersCount} players
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <Card className="bg-white/5 border-white/10">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg flex items-center gap-2">
              <span>🎤</span> Local Microphone ({activeMicPlayers})
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-white/5 border-white/10">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg flex items-center gap-2">
              <span>📱</span> Companion App ({activeCompanionPlayers})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <ScrollArea className="h-48">
              <div className="grid grid-cols-2 gap-2">
                {game.players
                  .filter(p => p.playerType === 'companion')
                  .map(player => (
                    <div
                      key={player.id}
