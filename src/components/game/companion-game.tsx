...
import type { CptmRoundResult } from './cptm-types';
...
export function CompanionGameView({
  ...
  const recordRound = useCallback(() => {
    ...
    for (const p of playersRef.current) {
      round.playerScores[p.id] = {
        ...
        notesMissed: p.notesMissed, maxCombo: p.maxCombo, segmentsSung: 0,
      };
    }
    setCptmSeriesHistory([...cptmSeriesHistory, round]);
  }, [song, cptmSeriesHistory, setCptmSeriesHistory]);
  ...
  const handleContinue = useCallback(() => {
    const resetPlayers = playersRef.current.map(p => ({
      ...p, ...EMPTY_PLAYER_SCORE, turnCount: 0, segmentsSung: 0,
    }));
    setCptmPlayers(resetPlayers);
    // Reset song so the library/pick screen shows again (CP-C2)
    setCptmSong(null);
    setPhase('intro');
    // Do NOT call onEndGame() — it wipes cptmPlayers via party-store.
  // eslint-disable-next-line react-hooks/exhaustive-deps -- setCptmPlayers/setCptmSong excluded; stable setState
  }, []);
          ...
          <Card className="bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/30 max-w-md w-full mb-6">
            <CardContent className="py-8 text-center">
              ...
              <div className="text-sm text-white/40">
                ...
                {cptmSeriesHistory.length > 0 && (
                  <span> • {t('companion.round').replace('{n}', String(cptmSeriesHistory.length + 1))}</span>
                )}
              </div>
