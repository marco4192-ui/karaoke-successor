import type { PartyGameConfig, SongSelectionOption } from './unified-party-setup.types';

// ===================== DEFAULT GAME CONFIGURATIONS =====================

export const PARTY_GAME_CONFIGS: Record<string, PartyGameConfig> = {
  'pass-the-mic': {
    mode: 'pass-the-mic',
    title: 'Pass the Mic',
    icon: '🎤',
    description: 'Take turns singing parts of a song!',
    titleKey: 'gameModes.passTheMic.title',
    descriptionKey: 'gameModes.passTheMic.description',
    extendedDescriptionKey: 'passTheMic',
    color: 'from-cyan-500 to-blue-500',
    minPlayers: 2,
    maxPlayers: 8,
    settings: [],
    songSelectionOptions: ['random', 'library', 'vote', 'medley'],
    supportsCompanionApp: false,
    forceInputMode: 'microphone',
    sharedMic: true,
    deviceAssignmentMode: 'shared-mic',
  },
  'companion-singalong': {
    mode: 'companion-singalong',
    title: 'Companion Sing-A-Long',
    icon: '📱',
    description: 'Your phone randomly lights up - sing when it blinks!',
    titleKey: 'gameModes.companionSingalong.title',
    descriptionKey: 'gameModes.companionSingalong.description',
    extendedDescriptionKey: 'companionSingalong',
    color: 'from-emerald-500 to-teal-500',
    minPlayers: 2,
    maxPlayers: 8,
    settings: [
      { key: 'minTurnDuration', label: 'Min Turn Duration', labelKey: 'modeSettings.minTurnDuration', type: 'slider', min: 5, max: 30, step: 5, defaultValue: 15, unit: 's' },
      { key: 'maxTurnDuration', label: 'Max Turn Duration', labelKey: 'modeSettings.maxTurnDuration', type: 'slider', min: 30, max: 90, step: 5, defaultValue: 45, unit: 's' },
      { key: 'blinkWarning', label: 'Blink Warning', description: 'Warning time before switch', labelKey: 'modeSettings.blinkWarning', descriptionKey: 'modeSettings.blinkWarningDesc', type: 'slider', min: 1, max: 5, step: 1, defaultValue: 3, unit: 's' },
    ],
    songSelectionOptions: ['random', 'library', 'vote', 'medley'],
    supportsCompanionApp: true,
    forceInputMode: 'companion',
    deviceAssignmentMode: 'none',
  },
  'medley': {
    mode: 'medley',
    title: 'Medley Contest',
    icon: '🎵',
    description: 'FFA or Team mode — sing random song snippets!',
    titleKey: 'gameModes.medley.title',
    descriptionKey: 'gameModes.medley.description',
    extendedDescriptionKey: 'medley',
    color: 'from-purple-500 to-pink-500',
    minPlayers: 2,
    maxPlayers: 4,
    settings: [
      { key: 'playMode', label: 'Play Mode', description: 'FFA, Team or Elimination', labelKey: 'modeSettings.playMode', descriptionKey: 'modeSettings.playModeDesc', type: 'select', options: [
        { value: 'ffa', label: 'FFA (Everyone vs Everyone — 4 Players)', labelKey: 'modeSettings.ffa' },
        { value: 'team', label: 'Team (1v1 or 2v2)', labelKey: 'modeSettings.team' },
        { value: 'elimination', label: '💀 Elimination (2-4 Players)', labelKey: 'modeSettings.elimination' },
      ], defaultValue: 'ffa' },
      { key: 'teamSize', label: 'Team Size', description: 'For team mode', labelKey: 'modeSettings.teamSize', descriptionKey: 'modeSettings.teamSizeDesc', type: 'select', options: [
        { value: 1, label: '1v1 (5 Snippets)', labelKey: 'modeSettings.1v1Snippets' },
        { value: 2, label: '2v2 (4 Snippets)', labelKey: 'modeSettings.2v2Snippets' },
      ], defaultValue: 1 },
      { key: 'snippetDuration', label: 'Snippet Duration', labelKey: 'modeSettings.snippetDuration', type: 'slider', min: 15, max: 60, step: 5, defaultValue: 30, unit: 's' },
      { key: 'transitionTime', label: 'Transition Time', description: 'Time between snippets', labelKey: 'modeSettings.transitionTime', descriptionKey: 'modeSettings.transitionTimeDesc', type: 'slider', min: 1, max: 5, step: 1, defaultValue: 3, unit: 's' },
    ],
    songSelectionOptions: ['random'],
    supportsCompanionApp: true,
    deviceAssignmentMode: 'exclusive',
  },
  'tournament': {
    mode: 'tournament',
    title: 'Tournament Mode',
    icon: '🏆',
    description: 'Single elimination bracket - Sudden Death!',
    titleKey: 'gameModes.tournament.title',
    descriptionKey: 'gameModes.tournament.description',
    extendedDescriptionKey: 'tournament',
    color: 'from-amber-500 to-yellow-500',
    minPlayers: 2,
    maxPlayers: 32,
    settings: [
      { key: 'maxPlayers', label: 'Bracket Size', labelKey: 'modeSettings.bracketSize', type: 'select', options: [
        { value: 2, label: '2 - Duel', labelKey: 'modeSettings.bracket2' }, { value: 4, label: '4 Players', labelKey: 'modeSettings.bracket4' },
        { value: 8, label: '8 Players', labelKey: 'modeSettings.bracket8' }, { value: 16, label: '16 Players', labelKey: 'modeSettings.bracket16' }, { value: 32, label: '32 Players', labelKey: 'modeSettings.bracket32' },
      ], defaultValue: 8 },
      { key: 'shortMode', label: 'Short Mode', description: 'Each match lasts only 60 seconds', labelKey: 'modeSettings.shortMode', descriptionKey: 'modeSettings.shortModeDesc', type: 'toggle', defaultValue: true },
      { key: 'tournamentType', label: 'Tournament Type', labelKey: 'tournament.type', type: 'select', options: [
        { value: 'single', label: 'Single Elimination', labelKey: 'modeSettings.singleElimination' },
        { value: 'double', label: 'Double Elimination', labelKey: 'modeSettings.doubleElimination' },
      ], defaultValue: 'single' },
      { key: 'tiebreakMode', label: 'Tiebreak Mode', labelKey: 'tournament.tiebreak', type: 'select', options: [
        { value: 'coinflip', label: 'Coin Flip', labelKey: 'modeSettings.coinFlip' },
        { value: 'accuracy', label: 'Accuracy', labelKey: 'modeSettings.accuracy' },
        { value: 'combo', label: 'Max Combo', labelKey: 'modeSettings.maxCombo' },
        { value: 'goldenmic', label: 'Golden Mic', labelKey: 'modeSettings.goldenMic' },
      ], defaultValue: 'accuracy' },
      { key: 'dynamicDifficulty', label: 'Dynamic Difficulty', labelKey: 'tournament.dynamicDifficulty', type: 'toggle', defaultValue: false },
      { key: 'songSelectionMode', label: 'Song Selection Mode', labelKey: 'tournament.songSelection', type: 'select', options: [
        { value: 'random', label: 'Random', labelKey: 'modeSettings.random' },
        { value: 'vote', label: 'Vote', labelKey: 'modeSettings.vote' },
      ], defaultValue: 'random' },
      { key: 'seedingMode', label: 'Seeding Mode', labelKey: 'tournament.seeding', type: 'select', options: [
        { value: 'random', label: 'Random', labelKey: 'modeSettings.random' },
        { value: 'strength', label: 'By Strength', labelKey: 'modeSettings.byStrength' },
      ], defaultValue: 'random' },
    ],
    songSelectionOptions: ['random'],
    supportsCompanionApp: true,
    deviceAssignmentMode: 'flexible',
  },
  'battle-royale': {
    mode: 'battle-royale',
    title: 'Battle Royale',
    icon: '👑',
    description: 'All sing together - lowest score eliminated each round! Up to 24 players (4 mic + 20 companion)',
    titleKey: 'gameModes.battleRoyale.title',
    descriptionKey: 'gameModes.battleRoyale.description',
    extendedDescriptionKey: 'battleRoyale',
    color: 'from-red-600 to-pink-600',
    minPlayers: 2,
    maxPlayers: 24,
    settings: [
      // Ausscheide-Intervall (user request 2.2-R2): this SINGLE setting is the
      // elimination rhythm — Random/Vote rounds eliminate the weakest player
      // every N seconds while the song keeps playing (song end = pure round
      // change). In Medley rounds the same value is the ROUND BUDGET the
      // 30-second snippet count is derived from. No hardcoded intervals.
      { key: 'roundDuration', label: 'Elimination Interval', labelKey: 'modeSettings.brEliminationInterval', description: 'Weakest player eliminated every N seconds (Random/Vote); medley round budget', descriptionKey: 'modeSettings.brEliminationIntervalDesc', type: 'slider', min: 30, max: 180, step: 15, defaultValue: 60, unit: 's' },
      // Final round length (user request 2.2-R2): grand finale rounds and the
      // last 2-player duel are capped by this duration — NOT the full song.
      { key: 'finalRoundDuration', label: 'Final Round Duration', labelKey: 'modeSettings.finalRoundDuration', description: 'Length of the final round (last duel / grand finale rounds)', descriptionKey: 'modeSettings.finalRoundDurationDesc', type: 'slider', min: 60, max: 300, step: 30, defaultValue: 120, unit: 's' },
      // NOTE: Song selection method is controlled by the top-level songSelectionOptions
      // (random|vote|medley), NOT by an inner setting. The medleyMode toggle was REMOVED:
      // selecting 'medley' as the song method now enables medley rounds (user rule 6.3);
      // random/vote always play complete songs.
      { key: 'grandFinaleBestOf', label: 'Grand Finale', description: 'Final 2 players format', labelKey: 'modeSettings.grandFinale', descriptionKey: 'modeSettings.grandFinaleDesc', type: 'select', options: [
        { value: 1, label: 'Normal Final', labelKey: 'modeSettings.normalFinal' },
        { value: 3, label: '🏆 Best of 3', labelKey: 'modeSettings.bestOf3' },
        { value: 5, label: '🏆 Best of 5', labelKey: 'modeSettings.bestOf5' },
      ], defaultValue: 1 },
      { key: 'bountyEnabled', label: 'Bounty System', description: 'Score multiplier for hunting the leader', labelKey: 'modeSettings.bountySystem', descriptionKey: 'modeSettings.bountySystemDesc', type: 'toggle', defaultValue: true },
      { key: 'bountyMultiplier', label: 'Bounty Multiplier', labelKey: 'modeSettings.bountyMultiplier', type: 'slider', min: 1.2, max: 3, step: 0.1, defaultValue: 1.5, unit: 'x' },
      { key: 'escalatingDifficulty', label: 'Escalating Difficulty', description: 'Difficulty increases every 3 rounds', labelKey: 'modeSettings.escalatingDifficulty', descriptionKey: 'modeSettings.escalatingDifficultyDesc', type: 'toggle', defaultValue: false },
      { key: 'shrinkingTimer', label: 'Shrinking Timer', description: 'Rounds get shorter over time', labelKey: 'modeSettings.shrinkingTimer', descriptionKey: 'modeSettings.shrinkingTimerDesc', type: 'toggle', defaultValue: false },
      { key: 'noRepeatProtection', label: 'No-Repeat Protection', description: 'Prevent recently played songs', labelKey: 'modeSettings.noRepeatProtection', descriptionKey: 'modeSettings.noRepeatProtectionDesc', type: 'toggle', defaultValue: true },
    ],
    songSelectionOptions: ['random', 'vote', 'medley'],
    supportsCompanionApp: true,
    deviceAssignmentMode: 'exclusive',
  },
  'duel': {
    mode: 'duel',
    title: 'Duel Mode',
    icon: '⚔️',
    description: 'Two players compete head-to-head!',
    titleKey: 'gameModes.duel.title',
    descriptionKey: 'gameModes.duel.description',
    extendedDescriptionKey: 'duel',
    color: 'from-cyan-500 to-pink-500',
    minPlayers: 2,
    maxPlayers: 2,
    settings: [],
    songSelectionOptions: ['library', 'random', 'vote'],
    supportsCompanionApp: false,
    deviceAssignmentMode: 'flexible',
  },
  'blind': {
    mode: 'blind',
    title: 'Blind Karaoke',
    icon: '🙈',
    description: 'Musical passages go blind! Solo, Co-op, or Competitive.',
    titleKey: 'gameModes.blind.title',
    descriptionKey: 'gameModes.blind.description',
    extendedDescriptionKey: 'blind',
    color: 'from-green-500 to-teal-500',
    minPlayers: 1,
    maxPlayers: 4,
    settings: [
      { key: 'blindFrequency', label: 'Blind Frequency', description: 'How often passages go blind', labelKey: 'modeSettings.blindFrequency', descriptionKey: 'modeSettings.blindFrequencyDesc', type: 'select', options: [
        { value: 'light', label: 'Light (15%)', labelKey: 'modeSettings.blindLight' }, { value: 'normal', label: 'Normal (30%)', labelKey: 'modeSettings.blindNormal' },
        { value: 'hard', label: 'Hard (60%)', labelKey: 'modeSettings.blindHard' }, { value: 'insane', label: 'Insane (90%)', labelKey: 'modeSettings.blindInsane' },
      ], defaultValue: 'normal' },
      { key: 'hardcore', label: 'Hardcore Mode', description: 'Text hidden when notes visible, and vice versa', labelKey: 'modeSettings.hardcoreMode', descriptionKey: 'modeSettings.hardcoreModeDesc', type: 'toggle', defaultValue: false },
      { key: 'escalating', label: 'Escalating', description: 'Frequency increases each round', labelKey: 'modeSettings.escalating', descriptionKey: 'modeSettings.escalatingDesc', type: 'toggle', defaultValue: false },
      { key: 'bestOf', label: 'Best of', description: 'Number of rounds per player', labelKey: 'modeSettings.bestOf', descriptionKey: 'modeSettings.bestOfDesc', type: 'select', options: [
        { value: 1, label: '1 Round', labelKey: 'modeSettings.1Round' }, { value: 3, label: 'Best of 3', labelKey: 'modeSettings.bestOf3' },
        { value: 5, label: 'Best of 5', labelKey: 'modeSettings.bestOf5' }, { value: 7, label: 'Best of 7', labelKey: 'modeSettings.bestOf7' },
      ], defaultValue: 3 },
    ],
    songSelectionOptions: ['random'],
    supportsCompanionApp: true,
    deviceAssignmentMode: 'flexible',
  },
  'missing-words': {
    mode: 'missing-words',
    title: 'Missing Words',
    icon: '📝',
    description: 'Words or passages disappear! Solo, Co-op, or Competitive.',
    titleKey: 'gameModes.missingWords.title',
    descriptionKey: 'gameModes.missingWords.description',
    extendedDescriptionKey: 'missingWords',
    color: 'from-orange-500 to-red-500',
    minPlayers: 1,
    maxPlayers: 4,
    settings: [
      { key: 'missingWordFrequency', label: 'Missing Words Frequency', description: 'How much text is hidden', labelKey: 'modeSettings.missingWordFrequency', descriptionKey: 'modeSettings.missingWordFrequencyDesc', type: 'select', options: [
        { value: 'light', label: 'Light (15%)', labelKey: 'modeSettings.mwLight' }, { value: 'normal', label: 'Normal (30%)', labelKey: 'modeSettings.mwNormal' },
        { value: 'hard', label: 'Hard (60%)', labelKey: 'modeSettings.mwHard' }, { value: 'insane', label: 'Insane (90%)', labelKey: 'modeSettings.mwInsane' },
      ], defaultValue: 'normal' },
      { key: 'granularity', label: 'Hiding Mode', description: 'Words, passages, or both', labelKey: 'modeSettings.missingGranularity', descriptionKey: 'modeSettings.missingGranularityDesc', type: 'select', options: [
        { value: 'word', label: 'Words', labelKey: 'modeSettings.mwWords' }, { value: 'passage', label: 'Passages', labelKey: 'modeSettings.mwPassages' }, { value: 'both', label: 'Both', labelKey: 'modeSettings.mwBoth' },
      ], defaultValue: 'passage' },
      { key: 'hardcoreMissingWords', label: 'Hardcore Mode', description: 'Hidden words stay hidden until song ends', labelKey: 'modeSettings.mwHardcoreMode', descriptionKey: 'modeSettings.mwHardcoreModeDesc', type: 'toggle', defaultValue: false },
      { key: 'escalating', label: 'Escalating', description: 'Frequency increases each round', labelKey: 'modeSettings.mwEscalating', descriptionKey: 'modeSettings.mwEscalatingDesc', type: 'toggle', defaultValue: false },
      { key: 'bestOf', label: 'Best of', description: 'Number of rounds per player', labelKey: 'modeSettings.bestOf', descriptionKey: 'modeSettings.bestOfDesc', type: 'select', options: [
        { value: 1, label: '1 Round', labelKey: 'modeSettings.1Round' }, { value: 3, label: 'Best of 3', labelKey: 'modeSettings.bestOf3' },
        { value: 5, label: 'Best of 5', labelKey: 'modeSettings.bestOf5' }, { value: 7, label: 'Best of 7', labelKey: 'modeSettings.bestOf7' },
      ], defaultValue: 3 },
    ],
    songSelectionOptions: ['random'],
    supportsCompanionApp: true,
    deviceAssignmentMode: 'flexible',
  },
  'rate-my-song': {
    mode: 'rate-my-song',
    title: 'Rate my Song',
    icon: '⭐',
    description: 'Sing a song and let friends rate your performance!',
    titleKey: 'gameModes.rateMySong.title',
    descriptionKey: 'gameModes.rateMySong.description',
    extendedDescriptionKey: 'rateMySong',
    color: 'from-amber-500 to-orange-500',
    minPlayers: 1,
    maxPlayers: 2,
    settings: [
      { key: 'duration', label: 'Duration', labelKey: 'modeSettings.duration', type: 'select', options: [
        { value: 'short', label: 'Short (60s)', labelKey: 'modeSettings.short60s' },
        { value: 'normal', label: 'Normal', labelKey: 'modeSettings.normalDuration' },
      ], defaultValue: 'normal' },
      { key: 'seriesRounds', label: 'Rounds', labelKey: 'modeSettings.seriesRounds', type: 'select', options: [
        { value: 1, label: '1 Round', labelKey: 'modeSettings.round1' },
        { value: 3, label: '3 Rounds', labelKey: 'modeSettings.rounds3' },
        { value: 5, label: '5 Rounds', labelKey: 'modeSettings.rounds5' },
        { value: 7, label: '7 Rounds', labelKey: 'modeSettings.rounds7' },
      ], defaultValue: 1 },
      { key: 'categoriesEnabled', label: 'Categories', description: '4 rating categories instead of single score', labelKey: 'modeSettings.categories', descriptionKey: 'modeSettings.categoriesDesc', type: 'toggle', defaultValue: true },
      { key: 'challengesEnabled', label: 'Challenges', description: 'Random challenges before each round', labelKey: 'modeSettings.challenges', descriptionKey: 'modeSettings.challengesDesc', type: 'toggle', defaultValue: false },
      { key: 'bettingEnabled', label: 'Betting', description: 'Audience can predict', labelKey: 'modeSettings.betting', descriptionKey: 'modeSettings.bettingDesc', type: 'toggle', defaultValue: false },
    ],
    songSelectionOptions: ['random', 'library', 'vote'],
    supportsCompanionApp: true,
    deviceAssignmentMode: 'flexible',
  },
};

// ===================== SONG SELECTION BUTTONS CONFIG =====================

export const SONG_SELECTION_CONFIG: Record<SongSelectionOption, {
  icon: string; label: string; labelKey: string; description: string; descriptionKey: string; color: string;
}> = {
  library: { icon: '📚', label: 'From Library', labelKey: 'unifiedSetup.fromLibrary', description: 'Choose a song from your library', descriptionKey: 'unifiedSetup.fromLibraryDesc', color: 'bg-cyan-500 hover:bg-cyan-600' },
  random: { icon: '🎲', label: 'Random Song', labelKey: 'unifiedSetup.randomSong', description: 'Let the game pick a random song', descriptionKey: 'unifiedSetup.randomSongDesc', color: 'bg-purple-500 hover:bg-purple-600' },
  vote: { icon: '🗳️', label: 'Vote (3 Songs)', labelKey: 'unifiedSetup.voteSongs', description: '3 songs are suggested - vote for your favorite', descriptionKey: 'unifiedSetup.voteSongsDesc', color: 'bg-amber-500 hover:bg-amber-600' },
  medley: { icon: '🎵', label: 'Medley Mix', labelKey: 'unifiedSetup.medleyMix', description: 'Mix multiple songs together', descriptionKey: 'unifiedSetup.medleyMixDesc', color: 'bg-pink-500 hover:bg-pink-600' },
};
