...
export function DesktopChatPanel({ open, onClose }: DesktopChatPanelProps) {
  ...
  const handleAcceptChallenge = useCallback(async (msgId: string, challenge: NonNullable<ChatMessage['challenge']>) => {
    const player = players.find((p) => p.id === selectedPlayerId);
    if (!player || acceptingId) return;
    ...
    try {
      await fetch('/api/mobile', {
        ...
        body: JSON.stringify({
          ...
          payload: {
            messageId: msgId,
            acceptedByClientId: player.id,
            acceptedByName: player.name,
  ...
  }, [players, selectedPlayerId, acceptingId, fetchMessages]);
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex flex-col ${msg.isHost ? 'items-end' : 'items-start'}`}>
              ...
              <div className={`max-w-[90%] px-3 py-2 rounded-2xl text-sm ${
                  ...
                  <div className="mt-2 rounded-lg bg-white/5 border border-white/10 p-2.5">
                    ...
                    {msg.challenge.accepted ? (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => handleAcceptChallenge(msg.id, msg.challenge!)}
                ...
                {/* challengeAccepted system message */}
                {msg.text === 'challengeAccepted' && !msg.challenge && (
