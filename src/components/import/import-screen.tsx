...
function findDuplicates(scannedSongs: ScannedSong[], existingSongs: Song[]): DuplicateInfo[] {
  return scannedSongs.map((song, index) => {
    // Check for exact match (same title and artist)
    ...
    // Check for similar match (same title, different artist or vice versa)
...
export function ImportScreen({ onImport, onCancel }: ImportScreenProps) {
  ...
  const processUltrastarImport = async () => {
    ...
    try {
      ...
      try {
        if (audioFile) {
          await storeMedia(song.id, 'audio', audioFile);
          song.storedMedia = true;
        }
        if (videoFile) {
          await storeMedia(song.id, 'video', videoFile);
          song.storedMedia = true;
        }
        console.log(`[Import] Stored media for song ${song.id} in IndexedDB`);
      } catch (mediaErr) {
        console.warn('[Import] Failed to store media in IndexedDB:', mediaErr);
        // Continue without stored media - will use blob URLs for this session
      }
      ...
      try {
        ...
        try {
          if (scanned.audioFile) {
            await storeMedia(song.id, 'audio', scanned.audioFile);
            song.storedMedia = true;
          }
          if (scanned.videoFile) {
            await storeMedia(song.id, 'video', scanned.videoFile);
            song.storedMedia = true;
          }
          if (scanned.coverFile) {
            await storeMedia(song.id, 'cover', scanned.coverFile);
            song.storedMedia = true;
          }
          console.log(`[Import] Stored media for song ${song.id} in IndexedDB`);
        } catch (mediaErr) {
          console.warn(`[Import] Failed to store media for ${song.title}:`, mediaErr);
        }
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {isDuplicate && dupInfo.existingSong && (
                              <p className="text-xs text-yellow-400/70">
                                Existing: {dupInfo.existingSong.title} by {dupInfo.existingSong.artist}
                              </p>
                            )}
                          </div>
