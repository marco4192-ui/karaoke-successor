...
import { useMobileAudioStream, TransmissionMode } from '@/hooks/use-mobile-audio-stream';
...
export function MobileClientView() {
  ...
  // Audio streaming hook for non-battle-royale game modes
  const audioStream = useMobileAudioStream(clientId, {
    sampleRate: 22050,
    channels: 1,
    chunkInterval: 100,
  });
  ...
  // Start microphone and pitch detection OR audio streaming based on mode
  const startMicrophone = useCallback(async () => {
    if (!clientId) return;
    
    // Check current transmission mode from server
    try {
      const modeResponse = await fetch('/api/mobile?action=transmissionmode');
      const modeData = await modeResponse.json();
      const currentMode: TransmissionMode = modeData.transmissionMode || 'pitch-only';
      setTransmissionMode(currentMode);
      
      if (currentMode === 'audio-stream') {
        // Use audio streaming mode - real audio sent to PC
        setIsListening(true);
        await audioStream.startStreaming();
        return;
      }
    } catch {
      // Fallback to pitch-only mode
    }
    
    // Pitch-only mode (default for Battle Royale)
    try {
  ...
  }, [clientId, yinPitchDetection, gameState.isPlaying, gameState.songEnded, audioStream]);

  // Stop microphone (both pitch and audio stream modes)
