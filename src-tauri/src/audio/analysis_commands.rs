...
//! Progress/results are reported back to the frontend via Tauri Channel IPC,
//! which bypasses plugin:event ACL restrictions.
...
enum AnalysisCommand {
    Analyze {
        ...
        on_complete: Channel<PitchAnalysisResult>,
        on_error: Channel<String>,
    },
    DetectBpm {
        file_path: String,
        on_complete: Channel<BpmDetectionResult>,
        on_error: Channel<String>,
    },
    Shutdown,
}
...
fn run_analysis_thread(
    ...
    loop {
        match rx.recv_timeout(Duration::from_millis(100)) {
            Ok(AnalysisCommand::Analyze { file_path, options, on_progress, on_complete, on_error }) => {
            ...
            Ok(AnalysisCommand::DetectBpm { file_path, on_complete, on_error }) => {
                match decode_mono_f64(&file_path) {
                    Ok(decoded) => {
...
/// Results are returned asynchronously via Tauri Channels (bypasses ACL).
/// Progress is reported via the on_progress channel.
#[tauri::command]
...
/// Result is returned asynchronously via Tauri Channels (bypasses ACL).
#[tauri::command]
pub fn audio_detect_bpm(
    ...
    let analysis_state = app.state::<AnalysisState>();
    let tx = analysis_state.command_tx.lock().map_err(|e| e.to_string())?;
    tx.send(AnalysisCommand::DetectBpm {
        file_path,
        on_complete,
        on_error,
    })
